...
const SelectedImage = ({
  ...
  return <div className={clsx(!disableContainerStyles && classes.imgContainer)} onMouseEnter={onHoverEnter} onMouseLeave={onHoverLeave}>
      ...
      <div className={clsx(classes.buttonGroup, !hover && classes.hidden)}>
        ...
        {handleRemoveImg && <Button className={classes.button} onClick={handleRemoveImg}>
            <ClearIcon fontSize="small" />
          </Button>}
      </div>
      {open && <Lightbox backdropClosesModal images={[{
      src: image
    }]} isOpen onClose={onCloseImage} />}
    </div>;
};
