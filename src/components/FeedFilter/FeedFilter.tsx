...
class FeedFilter extends React.PureComponent<Props, State> {
  ...
  handleChangeClasses = selected => {
    const userClasses = selected.map(s => ({ ...s,
      label: s.className,
    ...
    return <>
        <Paper className={classes.root} elevation={0}>
          <div className={classes.filtersHeader}>
            <InputBase className={classes.input} startAdornment={<SearchIcon classes={{
            root: classes.searchIcon
          }} />} placeholder={courseDisplayName ? 'Search posts' : 'To search add some posts first'} value={searchValue} onChange={this.handleChangeQuery} endAdornment={searchValue !== '' && <InputAdornment position="end">
                    <IconButton aria-label="Toggle password visibility" onClick={() => onChangeSearch('')}>
                      <ClearIcon fontSize="small" />
                    </IconButton>
                  </InputAdornment>} />
            <Divider className={classes.divider} />
            <IconButton color="primary" className={classes.iconButton} aria-label="Refresh" onClick={onRefresh}>
              <RefreshIcon />
            </IconButton>
          </div>
          <div className={classes.filtersFooter}>
            <DateRange from={fromDate} to={toDate} onChange={onChangeDateRange} />
            <Tooltip id={9047} hidden={!expertMode} placement="right" text="You can sort posts by date and filter posts by type!">
              <TransparentButton aria-haspopup="true" aria-label="Filter" aria-owns={open ? 'filter-popper' : undefined} className={classes.filterButton} onClick={filterCount ? this.handleRemoveFilter('all') : this.handleClick} variant="outlined" compact>
            ...
            <div className={classes.filterButtonContainer}>
              ...
              return <Chip key={targetPostType[0].label} label={targetPostType[0].label} classes={{
                root: classes.filterTypeBadge
              }} style={{
                backgroundColor: targetPostType[0].color
              }} clickable onDelete={this.handleRemoveFilter(targetPostType[0].value)} deleteIcon={<ClearIcon className={classes.deleteFilterIcon} fontSize="small" />} />;
            })}
            </div>
          </div>
        </Paper>
        <Dialog className={classes.dialog} okTitle="Search" onCancel={this.handleClose} onOk={this.handleApplyFilters} open={open} showBackIcon showActions showCancel okButtonClass={classes.searchButton} disableOk={!isPostTypesSelected} closeButtonClass={classes.closeSearchModalButton} title="Filter Class Feed">
          <Grid container>
            {!newClassExperience && <Grid item xs={12} sm={6} className={classes.option}>
                <FormControl className={classes.formControl}>
                  <FormLabel component="legend">Courses</FormLabel>
                  <FormGroup>
                    {classesList.map(item => <FormControlLabel key={item.label} control={<Checkbox checked={userClasses.findIndex(o => o === item.value) > -1} onChange={this.handleChange('userClasses')} value={item.value} />} label={item.label} />)}
                  </FormGroup>
                </FormControl>

                {isUserClassesSelected ? <Button color="primary" className={classes.formButton} onClick={this.handleDeselectAll('userClasses')}>
                    Deselect All
                  </Button> : <Button color="primary" className={classes.formButton} onClick={this.handleSelectAll('userClasses')}>
                    Select All
                  </Button>}
              </Grid>}
            <Grid item xs={12} className={classes.option}>
              <FormControl className={classes.formControl}>
                ...
                <FormGroup>
                  {types.map(type => <>
                      <FormControlLabel key={type.label} control={<Checkbox checked={postTypes.findIndex(o => o === type.value) > -1} checkedIcon={<span className={clsx(classes.icon, classes.checkedIcon)} />} icon={<span className={classes.icon} />} onChange={this.handleChange('postTypes')} value={type.value} />} label={type.label} />
                      <span className={classes.description}>{type.description}</span>
                    </>)}
                </FormGroup>
                <FormControlLabel key="deselect" control={<Checkbox checked={isPostTypesSelected} checkedIcon={<span className={clsx(classes.icon, classes.checkedIcon)} />} icon={<span className={classes.icon} />} onChange={isPostTypesSelected ? this.handleDeselectAll('postTypes') : this.handleSelectAll('postTypes')} value={isPostTypesSelected} />} label={isPostTypesSelected ? 'Deselect All' : 'Select All'} />
              </FormControl>
            </Grid>
          </Grid>
        </Dialog>
