...
const WorkflowBoardBox = ({
  ...
  const newInput = useMemo(() => <div className={classes.inputContainer}>
        {!newInputValue && <Typography className={classes.placeholder}>Enter a title for this task</Typography>}
        <TextField fullWidth className={classes.textField} InputProps={{
      className: classes.multilineColor
    }} autoFocus multiline rowsMax={3} onChange={handleChange} onKeyDown={handleKeyDown} />
      </div>, [handleKeyDown, handleChange, classes, newInputValue]);
  ...
  return useMemo(() => <Grid ref={drop} className={classes.container} style={{
    backgroundColor: bgcolor
        ...
        }} className={classes.button} onClick={openNew}>
              + Add a task
            </Button>
          </Grid>
        </Grid>
        <Grid container className={containerStyle}>
          <div className={classes.newContainer}>
            {showNew && <WorkflowBoardCard newInput={newInput} handleNew={handleNew} closeNew={closeNew} />}
          </div>
          {list}
        </Grid>
      </Grid>, [bgcolor, buttonColor, classes.button, classes.container, classes.headerItem, classes.newContainer, classes.title, closeNew, containerStyle, drop, handleNew, list, name, newInput, openNew, showNew]);
};
