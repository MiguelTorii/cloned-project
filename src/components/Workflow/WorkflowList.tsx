...
const WorkflowList = () => {
  ...
  const indexed = useMemo(() => tasks.map((t, index) => ({ ...t,
    index
  })), [tasks]);
  ...
  const boxes = workflowCategories.map((w, k) => ({ ...w,
    tasks: taskLists[k]
  }));
  return <div className={cx(!listView && classes.container)}>
      <DragPreview />
      ...
      <div className={cx(!listView && classes.columnContainer)}>
        {boxes.map(box => <WorkflowBox key={box.name} tasks={box.tasks} bgcolor={box.bgcolor} buttonColor={box.buttonColor} categoryId={box.categoryId} name={box.name} />)}
      </div>
    </div>;
};
