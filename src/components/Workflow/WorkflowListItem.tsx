...
const WorkflowListItem = ({
  ...
  onOpen = noOp,
  showDetails = false,
  isDragging = false // handleComplete,
  ...
  const renderTask = useMemo(() => {
    ...
    return <Grid container alignItems="center">
        ...
        <Grid item xs={3} md={3} className={classes.itemDetails}>
          {selected && <Chip label={selected.className} size="small" style={{
  ...
  return useMemo(() => <ListItem className={classes.item} selected={showDetails} dense>
      ...
      </ListItem>, [classes.dragContainer, classes.dragIcon, classes.item, isDragging, onOpen, renderTask, showDetails]);
};
