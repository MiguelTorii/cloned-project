...
const WorkflowBox = ({
  ...
  const onExpand = useCallback(() => handleExpand(categoryId)(!isExpanded), [handleExpand, categoryId, isExpanded]);
  const renderList = useMemo(() => {
    ...
    return <List className={classes.list} style={height}>
        {tasks.map((t, i) => {
        if (listView && i !== 0) {
          space += 32;
        } else {
        ...
        return <Motion key={t.id} defaultStyle={{
          scale: 0,
          y: space
        }} style={{
          y: spring(space, {
          ...
          scale: spring(1, {
            ...
            return <div style={moveStyle}>
                    <WorkflowItem // interpolatingStyle={interpolatingStyle}
              index={t.index} key={t.id} task={t} />
                  </div>;
          }}
            </Motion>;
      })}
  ...
  return useMemo(() => listView ? <WorkflowListBox list={renderList} drop={drop} name={name} tasks={tasks} isExpanded={isExpanded} onExpand={onExpand} /> : <WorkflowBoardBox list={renderList} drop={drop} bgcolor={bgcolor} buttonColor={buttonColor} name={name} categoryId={categoryId} tasks={tasks} />, [bgcolor, buttonColor, categoryId, drop, isExpanded, listView, name, onExpand, renderList, tasks]);
};
