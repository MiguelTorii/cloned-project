...
const WorkflowImageUpload = React.forwardRef(({
  imagesProps
}, ref) => {
  const {
    userId,
  ...
  const openImage = useCallback(idx => {
    setCurrentImage(idx);
  }, []);
  const handleImageClose = useCallback(() => setCurrentImage(null), []);
  const lightboxImages = useMemo(() => images.map(im => ({
    src: im.image
  })), [images]);
  ...
  return <div>
      <Dropzone accept={['image/*', 'application/pdf']} multiple onDrop={onDrop} onDropRejected={onDropRejected} disabled={disabled} maxSize={4 * 10000000}>
        {({
        getRootProps,
        getInputProps
      }) => <section>
            <div {...getRootProps()}>
              ...
              <div className={classes.dropZone}>
                <div className={classes.dropLabel} align="center">
                  {loading ? <CircularProgress size={50} /> : <Grid container alignItems="center" justifyContent="center" spacing={2} className={classes.uploadContainer}>
                      <Grid item>
      ...
      <Grid container className={classes.imagesContainer}>
        {images.map((im, idx) => <Grid item key={im.image} className={classes.imageContainer}>
      ...
      <Lightbox images={lightboxImages} currentImage={currentImage} isOpen={currentImage !== null} onClose={handleImageClose} onClickPrev={prev} onClickNext={next} backdropClosesModal />
    </div>;
});
