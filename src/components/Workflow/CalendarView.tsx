...
const CalendarView = () => {
  ...
  useEffect(() => {
    ...
    const tasksEmptyDate = calendarTasks.filter(t => String(t.date) === 'Invalid Date').map(t => ({ ...t,
      create: false
    }));
  ...
  return <div className={classes.root}>
      {currentTask && <Dialog className={classes.dialog} onCancel={closeConfirmArchive} open={confirmArchive} onOk={archive} showActions title="Are you sure you want to delete?" okTitle="Delete" showCancel>
          <Typography className={classes.archiveTitle}>
            {currentTask && currentTask.title}
          </Typography>
        </Dialog>}
      {currentTask && <WorkflowEdit task={currentTask} openConfirmArchive={openConfirmArchive} onClose={onCloseEdit} open />}
      <CalendarControls calendar={calendarRef.current} addTask={addTask} currentCalendarView={currentCalendarView} tasksEmptyDate={tasksEmptyDate} onOpenEdit={onOpenEdit} />
      <FullCalendar viewDidMount={onViewChange} ref={calendarRef} themeSystem="bootstrap" slotMinTime="06:00:00" slotMaxTime="20:00:00" defaultTimedEventDuration="00:00" drop={onExternalDrop} eventDrop={onDrop} eventClick={onOpenEdit} dateClick={onDateClick} allDaySlot={false} editable contentHeight="auto" dayMaxEventRows views={{
      dayGrid: {
        dayMaxEventRows: 3
      }
    }} droppable eventDurationEditable={false} initialView={currentCalendarView} nowIndicator headerToolbar={{
      left: '',
      center: '',
      right: ''
    }} plugins={[dayGridPlugin, interactionPlugin, timeGridPlugin, listPlugin]} events={calendarTasks} />
    </div>;
};
