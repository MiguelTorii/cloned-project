...
const getNotificationOptions = (seconds, updated, due) => {
  if (seconds && updated && due) {
    ...
    const option = Object.keys(remiderTime).reduce((prev: any, cur: any) =>
  ...
  useEffect(() => {
    ...
    if (task.date) {
      if (typeof task.date.getMonth === 'function') {
        ...
        setDate(new Date(`${task.date.replace(' ', 'T')}Z`) as any);
      }
    }
  }, [task, setImages, open]);
  useEffect(() => {
    ...
    >
      <Grid container spacing={2} className={classes.container}>
        ...
        <Grid item xs={12} md={8}>
          <DateInput selected={date as any} onChange={updateDate} />
        </Grid>
