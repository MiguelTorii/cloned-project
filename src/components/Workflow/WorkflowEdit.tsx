...
const getNotificationTime = notifications => {
  try {
    if (notifications && notifications.length === 1) {
      const {
        value
      } = remiderTime[notifications[0].key];
      return value;
    }
  } catch (e) {
    return null;
...
const getNotificationOptions = (seconds, updated, due) => {
  if (seconds && updated && due) {
    ...
    const option = Object.keys(remiderTime).reduce((prev, cur) => Math.abs(cur - optionSeconds) < Math.abs(prev - optionSeconds) ? cur : prev);
    return option;
  }
...
const WorkflowEdit = ({
  ...
  useEffect(() => {
    const notificationValue = getNotificationOptions(task.firstNotificationSeconds, task.notificationLastUpdated, date);
  ...
  const editNotification = useCallback((e, index) => {
    ...
    if (key === 'custom') {
      setNotifications(n => n.map((v, i) => i === index ? {
        key: key + index,
        value
      } : v));
      return;
  ...
  return <Dialog className={classes.dialog} contentClassName={classes.dialogContent} onCancel={onClose} open={open} onOk={updateTask} secondaryRemoveTitle="Delete" onSecondaryRemove={openConfirmArchive} showActions okTitle="Save">
      <Grid container spacing={2} className={classes.container}>
        <Grid item xs={12}>
          <TextField placeholder="Enter a task" inputProps={{
          className: classes.title
        }} fullWidth multiline onChange={updateTitle} value={title} size="small" />
        </Grid>
        <Grid item xs={12} className={classes.richText}>
          <ToolbarTooltip toolbar={additionalDetailToolbar} />
          <RichTextEditor setEditor={setEditor} placeholder="Additional Details" value={description} onChange={updateDescription} />
        </Grid>
        ...
        <Grid item xs={12} md={4}>
          <FormControl className={classes.selectForm}>
            ...
            <Select className={classes.select} value={categoryId} fullWidth onChange={updateType}>
              {workflowCategories.map(w => <MenuItem key={`cat-${w.name}`} value={w.categoryId}>
                  {w.name}
                </MenuItem>)}
            </Select>
          </FormControl>
        ...
        <Grid item xs={12}>
          {isFuture(date) && notifications && notifications.map((n, index) => <Notification n={n} key={`reminders-${n.key}`} dueDate={date} index={index} editNotification={editNotification} deleteNotification={deleteNotification} />)}
        </Grid>
        <Grid item xs={12}>
          <FormControl className={classes.selectForm}>
