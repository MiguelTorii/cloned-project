...
const FeedItem = ({
    ...
    switch (data.typeId) {
      case FeedTypes.flashcards.id:
        if (!newClassExperience) {
          return <div className={classes.flashCardsImage}>
        ...
        return <Box mt={3}>
            <Box className={classes.flashCards}>
              ...
              <Box mt={showSimple ? 2 : 3} pl={1} pr={1}>
                {!showSimple && <Typography variant="h6" className={classes.flashcardTitle}>
                    {data.title}
                  </Typography>}
                <Typography className={classes.flashcardCount} align={showSimple ? 'left' : 'center'}>
      ...
      case FeedTypes.note.id:
        if (!newClassExperience) {
          return <div className={classes.imageContainer}>
              {isPdf ? <PdfComponent url={data.noteUrl} height={75} width={75} radius={10} /> : <Image noLazyLoad className={classes.notePost} src={data.noteUrl} />}
              {numberOfNotes > 1 && <div className={classes.numberOfCardsStyle}>+{numberOfNotes - 1}</div>}
            </div>;
        }

        return <div className={classes.photoNotes}>
            {data.notes.slice(0, 1).map((note, i) => <div key={note.noteUrl} className={classes.photoNotePreview}>
                {isPdf ? <PdfComponent url={note.noteUrl} height={130} width={270} radius={10} /> : <Image noLazyLoad className={classes.notePost} src={note.noteUrl} />}
                <div className={classes.noteTitleBox}>{data.title}</div>
                {numberOfNotes > 2 && i === 1 && <div className={classes.numberOfCardsStyle}>+{numberOfNotes - 2}</div>}
              </div>)}
          </div>;
  ...
  const renderMenu = useMemo(() => <Menu disableAutoFocusItem anchorEl={moreAnchorEl} anchorOrigin={{
    vertical: 'top',
    horizontal: 'right'
  }} transformOrigin={{
    vertical: 'top',
    horizontal: 'right'
        ...
        {data.userId !== currentUserId ? <MenuItem onClick={handleReport}>
            <ListItemIcon color="inherit">
              <FlagIcon />
            </ListItemIcon>
            <ListItemText inset primary="Report an Issue" />
          </MenuItem> : isCurrent && <div>
      ...
      </Menu>, [currentUserId, data.userId, handleDelete, handleEdit, handleMenuClose, handleReport, isMenuOpen, moreAnchorEl, isCurrent]);
  return <Card className={classes.card} elevation={0} classes={{
    root: classes.root
  }}>
      <CardHeader className={classes.header} avatar={<Avatar src={feedTypeData.url} />} action={<>
            <IconButton aria-label="Share" onClick={handleShareLinkClick}>
              <ShareIcon />
            </IconButton>
            {newClassExperience && <IconButton aria-label="Bookmark" onClick={handleBookmark}>
                {data.bookmarked ? <BookmarkIcon className={classes.bookmarked} /> : <BookmarkBorderIcon />}
              </IconButton>}
            {(data.userId !== currentUserId || isCurrent) && <IconButton onClick={handleMenuOpen}>
                <MoreVertIcon />
              </IconButton>}
          ...
          </div>} subheader={<Typography className={classes.feedSubheader} variant={showSimple ? 'subtitle1' : 'body'}>
            {data.classroomName}
          </Typography>} />
      ...
      feedId: data.feedId
        ...
        {data.typeId !== FeedTypes.question.id && <CardContent className={classes.content}>
            {description && <div className={!data.title && description.length < 220 ? classes.titleFormat : classes.markdown}>
                {description}
              </div>}
            <span />
            {renderImage()}
          </CardContent>}
        {data.tags.length > 0 && <CardContent className={classes.tags}>
            {data.tags.map(tag => !newClassExperience ? <Chip key={tag.id} label={`#${tag.name}`} className={classes.chip} classes={{
          label: classes.label
      ...
      <CardActions className={classes.actions} disableactionspacing="true">
        <div className={classes.stats}>
          <Typography component="p" variant="subtitle1" className={cx(classes.actionIcons, thanked && classes.thankedMark)}>
            <IconButton aria-label="Share" disabled={currentUserId === ownerId} onClick={handleThanks}>
              {thanked ? <img src={thankedSvg} className={classes.actionIcon} alt="thanked" /> : <img src={thanksSvg} className={classes.actionIcon} alt="thanks" />}
            </IconButton>
        ...
        <Typography component="p" variant="subtitle1" className={!newClassExperience ? classes.stat : classes.stat2}>
          <strong>{data.postInfo.viewCount}</strong> &nbsp; views
        </Typography>
      </CardActions>
      {renderMenu}

      {showComments && <PostComments isPastClassFlashcard={pastClassIds.includes(data.classId) && data.typeId === FeedTypes.flashcards.id} feedId={data.feedId} postId={data.postId} typeId={data.typeId} classId={data.classId} toolbarPrefix={toolbarPrefix} isQuestion={data.typeId === FeedTypes.question.id} isOwner={data.userId === currentUserId} hasBestAnswer={data.bestAnswer} isCurrent={isCurrent} />}
    </Card>;
};
