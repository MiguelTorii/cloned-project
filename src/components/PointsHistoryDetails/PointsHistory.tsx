...
const PointsHistory = ({
  ...
  const fetchRecords = count => {
    dispatch(getPointsHistory(userId, records.length, count, points_history => {
  ...
  const renderRecords = isMobile => <Grid container spacing={isMobile ? 2 : 3}>
      {records.map((record, index) => <Grid key={index} item xs={12}>
  ...
  return <Paper className={classes.root}>
      ...
      <Hidden smDown>
        ...
        <InfiniteScroll className={classes.scroller} next={() => fetchRecords(UNITS_LOADING)} hasMore={hasMore} loader={<Box display="flex" justifyContent="center" mt={2} pt={2}>
              <CircularProgress />
            </Box>} dataLength={records.length}>
          {renderRecords(false)}
        </InfiniteScroll>
      </Hidden>
      <Hidden mdUp>
        ...
        <Box display="flex" justifyContent="center" mt={2}>
          <GradientButton loading={isLoadingData} disabled={isLoadingData} onClick={() => fetchRecords(UNITS_LOADING)}>
            See More
          </GradientButton>
        </Box>
      </Hidden>
