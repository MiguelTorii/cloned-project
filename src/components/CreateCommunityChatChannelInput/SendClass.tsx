...
const CreateChatChannelInput = ({
  ...
  useEffect(() => {
    ...
    selectedClasses.forEach(selectedClass => {
      const communityChannel = communityChannels.filter(communityChanel => communityChanel.courseId === selectedClass.community.id);
      const {
        channels
      } = communityChannel[0];
  ...
  const currentCommunities = useMemo(() => communities.filter(community => community.permissions.indexOf(PERMISSIONS.ONE_TOUCH_SEND_CHAT) > -1), []);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      enqueueSnackbarAction({
        notification: {
          message: `You successfully sent a message to ${selectedClasses[selectedClasses.length - 1]?.community.name}: ${selectChannel}`,
          options: {
            variant: 'info',
            anchorOrigin: {
              vertical: 'bottom',
      ...
      enqueueSnackbarAction({
        notification: {
          message: `There was an error sending your message to ${selectedClasses[selectedClasses.length - 1]?.community.name}: ${selectChannel}`,
          options: {
            variant: 'default',
            anchorOrigin: {
              vertical: 'bottom',
  ...
  }, [value, local, channels, chatIds, onClosePopover, selectedClasses, setCurrentCommunityId, setOneTouchSend, setCurrentCommunity, enqueueSnackbarAction]);
  ...
  return <>
      ...
      <Autocomplete multiple id="select-classes" options={currentCommunities} getOptionLabel={option => option.community.name} defaultValue={selectedClasses} noOptionsText="No Class" filterSelectedOptions classes={{
      root: classes.autoComplete,
      inputRoot: classes.inputRoot,
      noOptions: classes.noOptions
    }} onChange={handleSelectedClasses} renderTags={(value, getTagProps) => value.map((option, index) => <Chip key={option.community.id} classes={{
      root: classes.chip
    }} label={option.community.name} size="small" deleteIcon={<CloseIcon />} {...getTagProps({
      index
    })} />)} renderInput={params => <>
            <ClassIcon className={classes.classIcon} />
            <TextField {...params} className={selectedClasses.length ? classes.selectClass : classes.emptySelectedClass} variant="outlined" label={selectedClasses.length ? 'Class' : 'Select one or multiple classes'} />
          </>} />
      <Autocomplete id="select-channel" options={templateChannels.map(option => option.chat_name)} noOptionsText="No Channel" classes={{
      root: classes.autoComplete,
      inputRoot: classes.inputRoot,
      noOptions: classes.noOptions
    }} onChange={handleSelectedChannel} renderInput={params => <>
            <HashTag className={classes.hashIcon} />
            <TextField {...params} className={cx(selectChannel ? classes.selectClass : classes.emptySelectedClass)} variant="outlined" label={selectChannel ? 'Channel' : 'Select a channel'} />
          </>} />
      ...
      <Box display="flex" alignItems="center" justifyContent="flex-end" width={1}>
        ...
        {loading ? <Button className={classes.createDM} classes={{
        disabled: classes.loadingDisable
      }} disabled={loading} variant="contained" startIcon={<CircularProgress className={classes.loading} color="secondary" />} color="primary">
            Sending
          </Button> : <Button className={cx(showError ? classes.disabled : classes.createDM)} classes={{
        disabled: classes.disabled
      }} disabled={!chatIds.length || value === ''} variant="contained" onClick={showError ? handleRetry : handleSubmit} startIcon={showError && <ReplayIcon />} color="primary">
