...
const FlashcardsReview = ({
  ...
  const handleSetCurrentCardLevel = useCallback(level => {
    ...
    setCardsLevel({ ...cardsLevel,
      [card.id]: level
    });
  }, [currentCardIndex, currentCardList, cardsLevel, currentLevel, sessionId, flashcardId, reset, initializeTimer]);
  ...
  const renderSidebar = () => <Box className={clsx(classes.sidebar, !isExpanded && classes.unExpandedSidebar)}>
      ...
      <Grid container spacing={2}>
        {ANSWER_LEVELS.map(item => <Grid key={item.level} item xs={12}>
            <MarkCard title={item.title} mark={cardCountsByLevel[item.level]} markColor={item.color} active={currentLevel === item.level} onClick={() => handleMarkCardClick(item.level)} />
          </Grid>)}
      </Grid>
  ...
  const renderData = () => {
    ...
    return <>
        <Box mt={2} mb={2}>
          <CardBoard data={currentCardList[currentCardIndex]} onAction={handleSetCurrentCardLevel} />
        </Box>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            {currentLevel === CARDS_TO_REVIEW && <Button startIcon={<IconShuffle />} className={clsx(classes.shuffleButton, isShuffleOn && 'active')} onClick={handleShuffleDeck}>
          ...
          <Grid item>
            <Box display="flex">
              ...
              <Box ml={3}>
                <TransparentIconButton disabled={currentCardIndex >= currentCardList.length - 1} onClick={handleNextCard}>
                  <IconRight />
                </TransparentIconButton>
              </Box>
            </Box>
  ...
  return <Box display="flex" position="relative">
      <Slide in={isExpanded} direction="right">
        {renderSidebar()}
      </Slide>
      <Box className={clsx(classes.mainContent, isExpanded && 'expanded')}>{renderContent()}</Box>
      <Dialog okTitle="Yes" onCancel={handleCloseConfirmModal} onOk={handleResetProgress} open={isConfirmModalOpen} showActions showCancel title="Start Over">
