...
const CardBoardContent = ({
  ...
  return <div className={classes.cardBoardContainer}>
      {isEditing && <div className={classes.gradientBar} />}
      <div className={classes.cardBoardContent}>
        <Box display="flex" width="100%" height="100%" alignItems="center">
          {image && <Box width={contentText.length > 0 ? 250 : '100%'} height={contentText.length > 0 ? 250 : '100%'} display="flex" justifyContent="center" alignItems="center" mr={3}>
              <ClickableImage src={image} className={classes.cardBoardImage} alt="Flashcard" />
            </Box>}
          {contentText.length > 0 && <Box className={clsx(classes.cardBoardTextContainer, content.length < 200 && 'large-font')}>
              <div>{parse(content)}</div>
            </Box>}
        </Box>
      </div>
      {!isQuestion && <Box mt={2} mb={2}>
          <Box display="flex" justifyContent="center" alignItems="center">
            {ANSWER_LEVELS.map(item => <Box ml={2} mr={2} key={item.level}>
                <Button startIcon={<span>{item.emoji}</span>} className={classes.cardBoardActionButton} onClick={() => onAction(item.level)}>
                  {item.title}
                </Button>
              </Box>)}
          </Box>
      ...
      <div className={classes.cardBoardFooter}>
        <div></div>
        <Button size="large" endIcon={<IconRepeat />} classes={{
