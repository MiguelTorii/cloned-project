...
type Props = {
  ...
  isAnimating: boolean;
  isFlipped: boolean;
  onAnimationStart: (...args: Array<any>) => any;
  onAnimationEnd: (...args: Array<any>) => any;
  onShowAnswer: (...args: Array<any>) => any;
  onShowQuestion: (...args: Array<any>) => any;
  question: string;
...
const Flashcard = ({
  ...
  const prevIsAnimating = usePrevious({
    isAnimating
  });
  useEffect(() => {
  ...
  const Animation = ({
    ...
    return <AnimatedBox onAnimationEnd={handleAnimationEnd} x={x} y={y}>
        {children}
      </AnimatedBox>;
  };

  const renderAnswer = () => <Card ref={refCard} className={classes.body} key="back">
      <Animation>
        <CardContent className={classes.content}>
          {answerImage && <div className={classes.media}>
              <SelectedImage image={answerImage} className={classes.image} alt="flashcard answer" disableContainerStyles />
            </div>}
          {answer && <div className={classes.text}>
  ...
  const renderQuestion = () => <Card className={classes.body} key="front">
      <CardContent className={classes.content}>
        {questionImage && <div className={classes.media}>
            <SelectedImage image={questionImage} className={classes.image} alt="flashcard question" disableContainerStyles />
          </div>}
        {question && <div className={classes.text}>
  ...
  return <div>
    ...
    animationEnded ? null : <ReactCardFlip isFlipped={isFlipped}>
            {renderQuestion()}
            {renderAnswer()}
          </ReactCardFlip>}
      {!isFlipped ? <CardActions className={classes.actions}>
          <div className={classes.buttons}>
            <Button className={classes.button} color="primary" onClick={() => onShowAnswer(true)} variant="outlined">
              Show Answer
            </Button>
          </div>
        ...
        }} enterDelay={700} placement="top" title={<p className={classes.tooltipLabel}>
                  Select <b>{"Didn't Remember"}</b> {"if you weren't able to remember the answer"}
                </p>}>
              <Button className={classes.button} color="primary" onClick={() => handleAnswer('difficult')} variant="contained">
                {"Didn't Remember"}
              </Button>
            </Tooltip>
            <Tooltip arrow classes={{
          ...
          tooltip: classes.tooltip
              ...
              <Button className={classes.button} color="primary" onClick={() => handleAnswer('medium')} variant="contained">
                Almost Had It
              </Button>
            </Tooltip>
            <Tooltip arrow classes={{
          ...
          tooltip: classes.tooltip
              ...
              <Button className={classes.button} color="primary" onClick={() => handleAnswer('easy')} variant="contained">
                Correct!
              </Button>
            </Tooltip>
          </div>
          <div className={classes.buttons}>
            <Button className={classes.button} color="primary" onClick={() => onShowQuestion()} variant="outlined">
              Show Question
            </Button>
          </div>
        </CardActions>}
