...
const modules = {
  ...
  toolbar: [[{
    ...
    indent: '+1'
  }], ['formula'], ['image'], ['link'], ['clean'], ['emoji']],
  keyboard: {
    bindings: {
      tabToPost: {
        key: 13,
...
class CustomQuill extends React.PureComponent<Props> {
  ...
  render() {
    ...
    return <ReactQuill disabled className={cx(readOnly && classes.readOnly)} modules={modules} readOnly={readOnly} placeholder={placeholder} value={value} onChange={onChange} ref={element => {
      this.editor = element;
    }} />;
  }
