...
const FlashcardsDeckManager = ({
  ...
  const mergeEditorData = useCallback(data => data.map(card => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      if (!disableClass) {
        store.set(STORAGE_KEYS.FLASHCARD_CACHE, { ...formData,
  ...
  const handleSubmit = useCallback(() => {
    ...
    const data = mergeEditorData(deckData).filter(card => (card.questionImage || !!extractTextFromHtml(card.question)) && (card.answerImage || !!extractTextFromHtml(card.answer)));
  ...
  // Rendering Helpers
  const renderForm = () => <Box mt={4} mb={4}>
      <Grid container spacing={3}>
        <Grid item xs={12} lg={6}>
          <TextField required fullWidth error={isValidated && !formData.title} InputLabelProps={{
          shrink: true
        }} helperText={isValidated && !formData.title && 'Please input a title'} label="Title" placeholder="Add a title" value={formData.title || ''} onChange={event => handleUpdateField('title', event.target.value)} />
        </Grid>
        <Grid item xs={12} lg={6}>
          <TextField fullWidth required select error={isValidated && !formData.classId} InputLabelProps={{
          shrink: true
        }} label="Class" helperText={isValidated && !formData.classId && 'Please select your class'} placeholder="Select your class" disabled={disableClass} value={selectedDropdownValue} onChange={handleChangeClass}>
        ...
        <Grid item xs={12} lg={6}>
          ...
          shrink: true
        }} label="Description" placeholder="Add a description" value={formData.summary || ''} onChange={event => handleUpdateField('summary', event.target.value)} />
        </Grid>
        <Grid item xs={12} lg={6}>
  ...
  const ListEditor = useMemo(() => <FlashcardsListEditor onSetRef={handleSetRef} data={deckData} onUpdate={setDeckData} onUpdateFlashcardField={handleUpdateFlashcardField} />, [deckData, handleSetRef, handleUpdateFlashcardField]);
  return <Container>
