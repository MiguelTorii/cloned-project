...
import { BIG_CHAT_FILE_NAME_TRANCATE_LIMIT, SMALL_CHAT_FILE_NAME_TRANCATE_LIMIT } from "constants/chat";
...
const FileUploadContainer = ({
  ...
  return <Tooltip title={name} placement="top" arrow classes={{
    tooltip: classes.tooltip,
    arrow: classes.tooltipArrow,
    popper: smallChat ? classes.smallTitleTooltip : classes.titleTooltip
  }}>
      <div className={cx(smallChat ? classes.smallContainer : classes.container, isDownload && classes.download)} onMouseEnter={onMouseEnterHandler} onMouseLeave={onMouseLeaveHandler} onClick={() => downloadFile(name)}>
        ...
        <div className={classes.infoContainer}>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <div className={cx(smallChat ? classes.fileName : classes.name)}>
              {truncate(name, smallChat ? SMALL_CHAT_FILE_NAME_TRANCATE_LIMIT : BIG_CHAT_FILE_NAME_TRANCATE_LIMIT)}
            </div>
            {(width === 'xs' || smallChat) && isDownload && <DownloadIcon onClick={() => downloadFile(name)} className={classes.downloadIcon} />}
          </Box>
          {smallChat || width !== 'xs' && <Box display="flex" alignItems="center" className={classes.fileInfo}>
                <Typography variant="body2" component="div">
                  {size} {getFileContent(extension)}
                </Typography>
                {isDownload && <DownloadIcon onClick={() => downloadFile(name)} className={classes.downloadIcon} />}
              </Box>}
          {percentage > 0 && <div className={classes.progressBar}>
              <div className={classes.content} style={{
            width: `${percentage / 100 * 80}px`
          }} />
            </div>}
        </div>
      </div>
