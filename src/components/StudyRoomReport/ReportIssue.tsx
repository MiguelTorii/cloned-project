...
const ReportIssue = ({
  ...
  return <Dialog className={classes.dialog} onCancel={handleClose} open={open} showActions={false} contentClassName={classes.contentClassName} title={<div className={classes.title}>
      ...
      {!reported && <>
          <FormControl fullWidth variant="outlined" className={classes.selectForm}>
            ...
            <Select labelId="reporter-select-label" className={classes.select} value={selectedNames} fullWidth multiple onChange={handleSelectNames} MenuProps={{
          anchorOrigin: {
            vertical: 'bottom',
            horizontal: 'left'
          },
        ...
        }} renderValue={selected => <div className={classes.chipWrapper}>
                  {selected.map(value => <Chip className={classes.chip} key={value} label={getValue(value)} onMouseDown={e => {
            e.stopPropagation();
          }} onDelete={() => handleRemoveName(value)} />)}
                </div>}>
              {nameList.map(item => <MenuItem className={classes.menuItem} value={item.value} key={`name-item-${item.value}`}>
                  {selectedNames.indexOf(item.value) > -1 ? <CheckCircleIcon className={classes.mr1} /> : <RadioButtonUncheckedIcon className={classes.mr1} />}
                  {item.text}
                </MenuItem>)}
            </Select>
          ...
          <FormControl fullWidth variant="outlined" className={classes.selectForm}>
            ...
            <Select labelId="reason-select-label" className={classes.select} value={selectedReason} fullWidth onChange={handleSelectReason}>
              <MenuItem value="" className={classes.emptyOption} disabled />
              {reasonList.map(item => <MenuItem className={classes.menuItem} value={item.id} key={`reason-item-${item.id}`}>
                  {selectedReason === item.id ? <CheckCircleIcon className={classes.mr1} /> : <RadioButtonUncheckedIcon className={classes.mr1} />}
                  {item.reason}
                </MenuItem>)}
            </Select>
          </FormControl>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Button variant="contained" color="primary" className={classes.cancel} onClick={handleClose}>
              Cancel
            </Button>
      ...
      {reported && <>
          ...
          <Box display="flex" justifyContent="center" alignItems="center">
            <Button variant="contained" color="primary" className={classes.cancel} onClick={handleDoneClick}>
              Done
            </Button>
          </Box>
        </>}

      <SimpleErrorDialog open={openError} title={errorTitle} body={errorBody} handleClose={handleErrorDialogClose} />
    </Dialog>;
};
