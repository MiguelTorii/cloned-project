...
class AvailableRewardsItem extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    return <Paper key={rewardId} elevation={4} style={{
      backgroundColor: bgColor
    }} // classes={{ elevation24: classes.elevation }}
    className={cx(classes.item, isSelected && classes.selected)} onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseLeave}>
        ...
        {hover && <div className={classes.overlay}>
            ...
            {rewardCount > 1 && <Button color="primary" className={classes.button} onClick={this.handleClick(1)}>
                Move to second slot
              </Button>}
            {rewardCount > 2 && <Button color="primary" className={classes.button} onClick={this.handleClick(2)}>
                Move to third slot
              </Button>}
          </div>}
      </Paper>;
