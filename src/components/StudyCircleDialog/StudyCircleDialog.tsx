...
class StudyCircleDialog extends React.PureComponent<Props, State> {
  render() {
    ...
    return <Dialog ariaDescribedBy="study-circle-description" onCancel={onClose} open={open} title="Congratulations">
        <div className={classes.content}>
          ...
          {!loading && <div className={classes.circleContainer}>
              <div className={classes.main}>
                <Avatar src={userProfileUrl} className={classes.avatarBig}>
                  {initials}
                </Avatar>
              </div>
              {circle.map((item, index) => <div key={item.userId} className={classes.circle} style={{
            transform: `rotate(${270 + index * degreeAngle}deg) translate(120px) rotate(${-(270 + index * degreeAngle)}deg)`
          }}>
