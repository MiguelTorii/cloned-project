...
const CommunityMenu = ({
  ...
  useEffect(() => {
    ...
    if (id !== 'chat') {
      const currentCourseChannel = communityChannels.filter(courseChannel => courseChannel.courseId === id);
  ...
  return <Tooltip title={isDirectChat ? 'Direct Chat' : item.name} placement="top" arrow classes={{
    tooltip: classes.tooltip
  }}>
      {item.communityIconUrl ? <ListItem button onClick={handleSelectItem} selected={selectedCourse && selectedCourse.id === item.id} classes={{
      ...
      selected: classes.selectedItem
        ...
        </ListItem> : <ListItem button onClick={handleSelectItem} selected={selectedCourse && selectedCourse.id === item.id} classes={{
      root: classes.listItem,
      selected: classes.selectedItem
    }} style={{
      backgroundColor: item.color ? item.color : '#C45960'
    }}>
          {isDirectChat ? <StyledBadge max={99} badgeContent={unreadMessageCount} color="secondary">
              <ListItemIcon classes={{
          root: classes.itemContent
        }}>
                <Chat />
              </ListItemIcon>
            </StyledBadge> : <StyledBadge max={99} classes={{
        badge: unreadMessages ? classes.unreadMessageCount : classes.emptyUnreadMessage
      }} badgeContent={unreadMessages} color="secondary">
