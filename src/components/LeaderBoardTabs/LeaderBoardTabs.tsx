...
function TabPanel(props) {
  ...
  return <Typography component="div" role="tabpanel" hidden={value !== index} {...other}>
...
const LeaderBoardTabs = ({
  ...
  const renderPrizes = () => {
    ...
    if (window.innerWidth > 400) {
      return <div style={containerStyle}>
          {prizeImgs.map(img => <LoadImg key={img} url={img} style={imgStyle} />)}
  ...
  return <div className={classes.container}>
      <div className={classes.header}>
        <PrizeDialog eligibilitySubtitleDialog={eligibilitySubtitleDialog} handleCloseDialog={handleCloseDialog} openDialog={openDialog} amount={amount} numberOfWinners={numberOfWinners} dialogTitle={dialogTitle} eligibilityDialog={eligibilityDialog} />
        <Tabs variant="fullWidth" value={selectedTab} classes={{
        root: classes.fullWidth
      }} onChange={(e, n) => setSelectedTab(n)}>
          <Tab className="tour-onboarding-leaderboard-tuesday" classes={{
          wrapper: classes.tabs
        }} label={tuesdayBoardName} value="tuesday" />
          {showScholarshipTracker && <Tab className="tour-onboarding-leaderboard-grand" classes={{
          wrapper: classes.tabs
        }} label={grandBoardName} value="grand" />}
        </Tabs>
      </div>
          ...
          marginTop: 24
            ...
            display: 'flex'
              ...
              flexDirection: 'column'
                ...
                <div>
                  <Button className={classes.infoButton} onClick={handleOpenDialog} variant="outlined">
      ...
      <TabPanel value={selectedTab} index="tuesday">
        ...
        <div className={classes.rewardContainer}>
          ...
          <div>
            ...
            <Button variant="outlined" color="primary" onClick={selectedTab === 'grand' ? handleOpenDialog : navigateToStore} className={classes.button}>
              {rewardButtonText}
            </Button>
          </div>
        </div>
      ...
      <Table userId={Number(userId)} scoreLabel={scoreLabel} pushTo={pushTo} students={students} sectionId={sectionId} selectedTab={selectedTab} />
    </div>;
};
