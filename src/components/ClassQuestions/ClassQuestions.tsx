...
const ClassQuestions = ({
  ...
  useEffect(() => {
    const classes = (classData?.section || []).map(section => JSON.stringify({
  ...
  return <>
      <Grid container direction="column" spacing={3}>
        {feedList.map(item => <Grid item key={item.feedId}>
            <Paper elevation={0} square={false} className={classes.feedContainer}>
              <FeedItem expertMode={isExpertMode} userId={me.userId} schoolId={me.schoolId} data={item} handleShareClick={handleShare} onPostClick={handlePostClick} onBookmark={handleBookmark} onReport={handleReport} onDelete={handleDelete} onUserClick={handleUserClick} />
            </Paper>
          </Grid>)}
      </Grid>
      ...
      <DeletePost open={Boolean(deleteData)} feedId={(deleteData || {}).feedId || -1} onClose={handleDeleteClose} />
      <Report open={Boolean(reportData)} ownerId={(reportData || {}).ownerId || ''} objectId={(reportData || {}).feedId || -1} onClose={handleReportClose} />
      <SharePost feedId={Boolean(shareData)} open={(shareData || {}).feedId} onClose={handleShareClose} />
    </>;
};
