...
const ChatMessage = ({
  ...
  const handleViewProfile = userId => () => {
    history.push(buildPath(`/profile/${userId}`, {
  ...
  const renderItem = ({
    ...
    if (['add_new_member', 'remove_user'].includes(imageKey)) {
      return <div className={cx(classes.alertWrapper)}>
          <Typography className={classes.alert} dangerouslySetInnerHTML={{
    ...
    return <AnyFileUpload files={files} message={message} onImageClick={onImageClick} onImageLoaded={onImageLoaded} renderHtmlWithImage={renderHtmlWithImage} />;
  };

  return <>
      {messageList.map(message => <ListItem key={message.sid} alignItems="flex-start" className={classes.root} onMouseEnter={handleMouseEnter(message.sid)} onMouseLeave={handleMouseLeave()}>
          <div className={classes.content}>
            {message.imageKey !== 'add_new_member' && <ListItemAvatar className={classes.avatarLink} component={MyLink} href={buildPath(`/profile/${userId}`, {
          from: PROFILE_PAGE_SOURCE.CHAT
        })}>
          ...
          <div className={classes.message}>
            {message.imageKey !== 'add_new_member' && <div className={classes.messageHeader}>
                ...
                {showOpetions === message.sid && currentUserId !== userId && <Button className={classes.threeDots} variant="contained" onClick={handleClick(message.sid)}>
                    <MoreHorizIcon />
                  </Button>}
                <Popover id={message.sid} open={open && hoverMessageId === message.sid} anchorEl={anchorEl} onClose={handleClose} anchorOrigin={{
            vertical: 'bottom',
            horizontal: 'right'
          }} transformOrigin={{
            vertical: 'top',
            horizontal: 'right'
                  ...
                  <MenuList className={classes.userMenu}>
                    ...
                    {isGroupChannl && <MenuItem onClick={handleOpenBlockMemberModal(userId, name)}>
      ...
      <BlockMemberModal closeModal={handleCloseBlockMemberModal} handleBlock={handleBlock} onOk={handleBlockUser} open={openBlockModal} blockUserName={blockUserName} />
    </>;
};
