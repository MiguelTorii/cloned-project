...
class QuestItem extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    return <ButtonBase className={cx(classes.root, isCurrent && classes.current, isHidden && classes.hiden)} disabled={!isCurrent} component={MyLink} href={this.renderQuestLink(action)} // elevation={1}
    >
        ...
        <Typography variant="caption" className={cx(classes.body, isCurrent && classes.bodyCurrent)}>
          {task}
        </Typography>
        <div className={cx(classes.imageWrapper, !isCurrent && classes.imageWrapperSmall)}>
          <img src={iconUrl} alt="Quest" className={cx(classes.image, !isCurrent && classes.imageSmall)} />
        </div>
        {status === 'complete' && <div className={classes.completed}>
