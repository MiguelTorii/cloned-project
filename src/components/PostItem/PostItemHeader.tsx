  ...
  render() {
    ...
    const renderMenu = <Menu disableAutoFocusItem anchorEl={moreAnchorEl} anchorOrigin={{
      ...
      horizontal: 'right'
        ...
        {userId !== currentUserId ? <MenuItem onClick={this.handleReport}>
            <ListItemIcon color="inherit">
              <ReportIcon />
            </ListItemIcon>
            <ListItemText inset primary="Report" />
          </MenuItem> : <div>
            {!pastClassIds.includes(classId) && <MenuItem onClick={this.handleEdit}>
                <ListItemIcon color="inherit">
                  <CreateIcon />
                </ListItemIcon>
    ...
    return <>
        <Grid className={classes.backButton} container justifyContent="flex-start" alignItems="center" onClick={this.handleGoBack}>
          <ArrowBackIosRoundedIcon />
          <Typography className={classes.feedTypo}>{navigationTitle}</Typography>
        </Grid>
        <div className={classes.root}>
          <Link className={classes.avatar} component={MyLink} href={buildPath(`/profile/${userId}`, {
          ...
          <div className={classes.userInfo}>
            <Box display="flex" alignItems="center">
              <Typography component="p" variant="h6" noWrap>
                <Link component={MyLink} href={buildPath(`/profile/${userId}`, {
                from: PROFILE_PAGE_SOURCE.POST
            ...
            {!newClassExperience || expertMode ? <Typography component="p" variant="subtitle1" noWrap>
                {classroomName} • {fromNow}
              </Typography> : <Typography component="p" variant="subtitle1" noWrap>
                {fromNow}
              </Typography>}
          </div>
        ...
        {!isMarkdown ? <Typography className={classes.body} component="p" variant="h6">
            {showShortSummary ? _.truncate(body, {
          length: BODY_LENGTH_THRESHOLD
        }) : body}
            {body.length > BODY_LENGTH_THRESHOLD && <Link className={classes.moreLessLink} onClick={this.handleSummaryMoreOrLess} color="inherit" underline="always">
                {showShortSummary ? 'see more' : 'show less'}
              </Link>}
          </Typography> : <div className={classes.markdown}>
