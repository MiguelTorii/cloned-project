...
const FlashcardsDeck = ({
  ...
  // Memos
  const deckClass = useMemo(() => classList.find(item => item.classId === data.class_id), [data, classList]);
  const deckLink = useMemo(() => `${APP_ROOT_PATH}/flashcards/${data.post_id}`, [data]);
  const shareLinkModalTitle = useMemo(() => <Typography variant="h6">
  ...
  const handleBookmark = useCallback(() => {
    dispatch(bookmarkFlashcards(me.userId, data.feed_id, data.bookmarked));
  }, [data, me, dispatch]);
  ...
  // Rendering
  return <div className={cx(classes.root, search.includes('past') && classes.pastClassRoot)} onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
      <div className={classes.contentContainer}>
        <Box display="flex" flexDirection="column" justifyContent="space-between" height="100%" onClick={handleView}>
          <div>
          ...
          <div>
            {deckClass && <Chip label={_.truncate(deckClass.className, {
            length: 25
          })} size="small" style={{
            backgroundColor: deckClass.bgColor,
            color: 'white'
          }} />}
            <div aria-hidden="true" className={clsx(!isHover && classes.hidden)} style={{
            animation: animations.fadeIn
          }} onClick={handleClickActionBar}>
              <ActionBar isOwn={me.userId.toString() === data.user_id.toString()} bookmarked={data.bookmarked} onViewEdit={handleView} onBookmark={handleBookmark} onShareLink={handleShareLink} onDelete={handleOpenDeleteModal} />
            </div>
          </div>
        </Box>
        <ShareLinkModal open={isShareLinkModalOpen} link={deckLink} title={shareLinkModalTitle} onClose={handleCloseShareLinkModal} />
        <Dialog className={classes.deleteModal} open={isDeleteModalOpen} title="Delete Flashcard Deck" onCancel={handleCloseDeleteModal}>
