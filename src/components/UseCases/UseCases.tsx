...
const UseCases = ({
  ...
  const organizeText = useMemo(() => 'To make life easier, we all need reminders. Setup tasks and reminders to study, to review flashcards and to review notes your classmates posted', []);
  useEffect(() => {
    const init = async () => {
  ...
  return <div>
      <div className={classes.row}>
        <UseCase title="Need Help" text="It feels terrible to struggle and not have immediate help. Post a question, your classmates get notified, and when you vote a student with “Best Answer”, they get 25,000 points for helping you out.">
          <Item imageUrl={question} title="Post a Question" to="/create/question" />
          {videoEnabled && <Item imageUrl={videos} to="/video-call" title="Start a Chat or Group Chat" />}
        </UseCase>
        <UseCase title="Group Studying or Project" text="CircleIn makes group projects and studying so much easier. Don’t worry if someone is down the hall or across the country">
          {videoEnabled && <Item imageUrl={videos} to="/video-call" title="Create a Video Study Session" />}
      ...
      <div className={classes.row}>
        <UseCase title="Get Ready for a Test" text="CircleIn is great for preparing for your next midterm or final exam">
...
export default connect(mapStateToProps, mapDispatchToProps)(withRoot(withStyles(styles)(withWidth()(UseCases))));