...
type Props = {
  ...
  participants?: any;
  profiles?: any;
  lockedParticipant?: any;
  sharingTrackIds?: any;
  currentUserId?: any;
  dominantSpeaker?: any;
  viewMode?: any;
  selectedScreenShareId?: any;
  meetupRef?: any;
  isSharing?: boolean;
  onLockParticipant?: any;
};
...
const Thumbnails = ({
  ...
  return (
    <div className={cx(classes.root, viewMode === 'side-by-side' && classes.sideBySideRoot)}>
      {totalPageCount > 1 && (
        <Button
          onClick={goBack}
          size="small"
          className={cx(classes.prevPage, viewMode === 'side-by-side' && classes.prevPageSideView)}
          classes={{
            label: classes.labelButton
          }}
        >
          {viewMode === 'speaker-view' ? <ArrowBackIosIcon fontSize="small" /> : <ExpandLessIcon />}
          <Typography variant={'p' as any}>
            {selectedPage} / {totalPageCount}
          </Typography>
        </Button>
      )}
      ...
      {totalPageCount > 1 && (
        <Button
          onClick={goNext}
          size="small"
          className={cx(classes.nextPage, viewMode === 'side-by-side' && classes.nextPageSideView)}
          classes={{
            label: classes.labelButton
          }}
        >
...
export default withStyles(styles as any)(Thumbnails);
