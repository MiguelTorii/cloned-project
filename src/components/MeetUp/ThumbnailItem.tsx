...
type Props = {
  ...
  isMic: boolean;
  highlight: boolean;
  isLocal: boolean; // isDataSharing: boolean,
  // isSharing: boolean
...
class ThumbnailItem extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    const activeBorder = highlight && isScreenShare ? {
      border: '4px solid #03A9F4'
    } : {};
    return <Box onMouseOver={() => this.onMouseOver()} onMouseOut={() => this.onMouseOut()} className={cx(classes.root, isVisible && classes.hide)} style={{ ...activeBorder
    }}>
        <div className={classes.videoWrapper}>
          {isVideo ? <div className={cx(classes.video, isLocal && classes.cameraVideo)} ref={this.videoinput} /> : profileImage ? <div className={classes.avatar}>
        ...
        <div className={cx(!isMic ? classes.mic : hover && (firstName || lastName) ? classes.mic : classes.hide)}>
