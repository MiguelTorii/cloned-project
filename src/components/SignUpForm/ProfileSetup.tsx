...
type State = {
  ...
  // grade: string | number,
  // school: ?SelectType,
  studentId: string; // parentFirstName: string,
  ...
  render() {
    ...
    return <ValidatorForm onSubmit={this.handleSubmit} className={cx(classes.form, hide && classes.hide)}>
        <FormControl variant="outlined" fullWidth className={classes.formControl}>
          <SelectValidator value={state} name="state" label="State" onChange={this.handleChange('state')} variant="outlined" validators={['required']} errorMessages={['State is required']}>
            <MenuItem value="" />
            {(states[type] || []).map(item => <MenuItem key={item.value} value={item.value}>
                {item.label}
              </MenuItem>)}
          </SelectValidator>
        </FormControl>
        ...
        <TextValidator variant="outlined" label="Student ID" margin="normal" onChange={this.handleChange('studentId')} name="studentId" autoComplete="studentId" fullWidth value={studentId} disabled={loading} />
        {
