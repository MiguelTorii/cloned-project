...
const EditProfileModal = ({
  ...
  return <Dialog title="Edit Your Profile" open={open} onCancel={onClose} maxWidth="lg">
      <Grid container className={classes.root} spacing={4}>
        <Grid item xs={12}>
          <Box display="inline-block" position="relative">
            <Avatar mobileSize={DEFAULT_AVATAR_SIZE.desktop} src={image} initialText={getInitials(`${profile.firstName} ${profile.lastName}`)} />
            <Button onClick={() => setIsEditingAvatar(true)} classes={{
        ...
        <Grid item xs={12}>
          <TextField fullWidth multiline rows={3} InputLabelProps={{
          shrink: true
        }} value={bioText} onChange={event => event.target.value.length <= MAX_BIO_LENGTH && setBioText(event.target.value)} maxLength={MAX_BIO_LENGTH} label={`Bio(${MAX_BIO_LENGTH - bioText.length} characters remaining)`} placeholder="Add a quick description about you here! Your classmates will view your profile to learn more about you!" />
        </Grid>
        <Grid item xs={12}>
      ...
      <AvatarEditor open={isEditingAvatar} originalImage={image} onCancel={handleCancelEditAvatar} onSave={handleChangeAvatar} />
    </Dialog>;
};
