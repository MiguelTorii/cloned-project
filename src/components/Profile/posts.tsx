...
class ProfilePosts extends React.PureComponent<Props> {
  ...
  render() {
    ...
    if (posts.length === 0) {
      if (isBookmarks) {
        return <div className={cx(classes.container, classes.nothing)}>
      ...
      return <div className={cx(classes.container, classes.nothing)}>
          <Typography variant="h6" color="textPrimary" align="center">
            {isMyProfile ? 'You do not have any posts yet. Once you create a post you can come back here to view them' : "Please check back later. They haven't created any posts yet."}
          </Typography>
        </div>;
    }
    ...
    return <div className={classes.container}>
        <Paper className={classes.root} elevation={0}>
          {posts.map(item => <FeedItem key={item.feedId} postId={item.postId} typeId={item.typeId} userId={userId} data={item} handleShareClick={onShare} onPostClick={onPostClick} onBookmark={onBookmark} onReport={onReport} onDelete={onDelete} pushTo={pushTo} onUserClick={onUserClick} setQuillRefs={this.setQuillRefs} quillRefs={this.quillRefs} setNewComments={this.setNewComments} newComments={this.newComments} toolbarPrefix={isBookmarks ? 'bookmark' : ''} />)}
        </Paper>
      </div>;
  }
