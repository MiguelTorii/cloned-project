...
const RecommendationsFeedback = ({
  ...
  const renderRatingText = () => {
    ...
    return <>
        ...
        {ratingIndex === 3 && !gaveMore && <Box display="flex" justifyContent="space-between">
  ...
  return <Paper elevation={0} className={classes.root}>
      ...
      <Grid container spacing={2}>
        {ratings.map((item, index) => <Grid key={item.text} item xs={3} className={classes.imageContainer}>
            {ratingIndex !== null ? <img src={item.image} alt={item.text} className={clsx(classes.image, ratingIndex !== index && 'disabled')} /> : <Link component="button" onClick={() => handleGiveFeedback(index)}>
                <img src={item.image} alt={item.text} className={classes.image} />
              </Link>}
          </Grid>)}
      </Grid>
      {ratingIndex !== null ? renderRatingText() : <Box display="flex" justifyContent="space-between">
