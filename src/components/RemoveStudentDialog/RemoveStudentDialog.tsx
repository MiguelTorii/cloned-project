...
const RemoveStudentDialog = ({
  ...
  return <>
      <Dialog open={open} title="Remove Student" className={classes.dialog} hrClass={classes.hrClass} onCancel={onClose}>
        {isCommunityChat && <Paper component="form" className={classes.searchStudent}>
            <IconButton className={classes.iconButton} aria-label="menu">
              <ChatSearchIcon />
            </IconButton>
            <InputBase className={classes.input} onChange={searchStudentName} placeholder="Search for students" inputProps={{
          'aria-label': 'search for students'
        }} value={search} />
          </Paper>}
        <List dense className={classes.listRoot}>
          {members.filter(member => `${member.firstname} ${member.lastname}`.includes(search)).map(m => {
          const fullName = `${m.firstname} ${m.lastname}`;
          return <ListItem key={m.userId} disableGutters button classes={{
            secondaryAction: classes.secondaryAction
                  ...
                  <ListItemSecondaryAction>
                    {!isCommunityChat && removedUserIds.includes(Number(m.userId)) && <Button onClick={() => handleUndo(m.userId)} className={classes.undo}>
                        <UndoIcon className={classes.spacing} /> Undo
                      </Button>}
                    <Button onClick={() => removeMember(m)} className={cx(isCommunityChat ? classes.removeUser : removedUserIds.includes(Number(m.userId)) ? classes.removedUser : classes.removeUser)}>
                      {isCommunityChat ? 'Remove' : removedUserIds.includes(Number(m.userId)) ? 'Removed' : 'Remove'}
                    </Button>
                  </ListItemSecondaryAction>
                </ListItem>;
        })}
      ...
      <Dialog open={confirmDialog} title={`${user?.firstname} ${user?.lastname}?`} className={classes.dialog} hrClass={classes.hrClass} okButtonClass={classes.okButtonClass} closeButtonClass={classes.closeButtonClass} onCancel={handleCloseConfirmModal} onOk={removeUserFromChannel} showActions showCancel okTitle="Yes, Remove" cancelTitle="Cancel">
