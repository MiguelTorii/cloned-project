...
type Props = {
  ...
  flashcardId: number | null | undefined;
...
const CreateFlashcards = ({
  ...
  const clearFlashcards = useCallback(() => {
    ...
    setSummary('');
    setFlashcards([]);
    store.remove('FLASHCARDS_CACHE'); // closeConfirmDialog()
  }, []);
  ...
  const createFlashcards = useCallback(async () => {
    ...
    try {
      ...
      const {
        ...
        classes: resClasses,
        fcId
      } = canBatchPost ? await api.createBatchFlashcards({
        userId,
        title,
        summary,
        deck: flashcards.map(item => ({
  ...
  return <div className={classes.root}>
      <ErrorBoundary>
        ...
        <CreatePostForm title={`${flashcardId ? 'Edit' : 'Create'} Flashcards`} buttonLabel={flashcardId ? 'Update' : 'Create'} subtitle="Time to study! 🙌🏽 Flashcards are very helpful to learn formulas, new terms, study material for quizzes/homework, help your brain memorize new information and so much more. Plus you can add images!" loading={loading} changed={changed} handleSubmit={handleSubmit}>
          <Grid container alignItems="center">
            <Grid item xs={12} sm={12} md={2}>
              <Typography variant="subtitle1">Title</Typography>
            </Grid>
            <Grid item xs={12} sm={12} md={10}>
              <OutlinedTextValidator label="Title" onChange={handleTextChange} name="title" value={title} validators={['required']} errorMessages={['Title is required']} />
            </Grid>
            <Grid item xs={12} sm={12} md={2}>
              <Typography variant="subtitle1">Description</Typography>
            </Grid>
            <Grid item xs={12} sm={12} md={10}>
              <OutlinedTextValidator label="Description" onChange={handleTextChange} name="summary" multiline rows={4} value={summary} validators={['required']} errorMessages={['Description is required']} />
            </Grid>
            {
            ...
            <Grid item xs={12} sm={12} md={10} className={classes.flashcards}>
              {flashcards.map((flashcard, index) => <FlashcardEditor key={flashcard.id} isNew={flashcard.isNew} id={flashcard.id} index={index + 1} loading={loading} question={flashcard.question} answer={flashcard.answer} questionImage={flashcard.questionImage} answerImage={flashcard.answerImage} onDelete={handleDelete} onSubmit={handleUpdate} />)}
              <NewFlashcard error={flashcardsError && flashcards.length === 0} loading={loading} onClick={handleAddNew} />
              {
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(withRouter(CreateFlashcards)));