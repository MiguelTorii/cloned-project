...
const PostCreationHeader = () => {
  ...
  return <Box className={classes.postHeaderRoot}>
      <Paper className={classes.postHeaderPaper}>
        <Box display="flex" alignItems="center" pb={2}>
          <Box mr={3}>
            <Avatar src={me.profileImage} initialText={getInitials(`${me.firstName} ${me.lastName}`)} mobileSize={50} desktopSize={50} />
          </Box>
          <Box flexGrow={1}>
            <Tooltip id={9090} placement="bottom" text="Earn points and earn money. A great way to do that is by creating a post." okButton="Yay! 🎉">
        ...
        <Grid container spacing={3}>
          {POST_BUTTONS.map(item => <Grid item xs={6} lg={3} key={item.value}>
              <Button className={classes.postButton} startIcon={<img src={item.icon} alt={item.value} />} onClick={() => handleGotoPostCreate(item.value)}>
                {item.text}
              </Button>
            </Grid>)}
        </Grid>
