...
type Props = {
  ...
  updateFilterFields: (...args: Array<any>) => any;
  actionFetchFeed: (...args: Array<any>) => any;
  deleteFeed: (...args: Array<any>) => any;
};
type State = {
  activeAction: {
    type: string;
...
class Feed extends React.PureComponent<Props, State> {
  ...
  componentDidMount = () => {
    ...
    if (sectionId) {
      ...
      filter.userClasses = (classList || []).map(classData => classData.section?.[0].sectionId).filter(sectionId => !!sectionId);
    }
  ...
  render() {
    ...
    const selectedClasses = classList.filter(classData => userClasses.includes(classData.section?.[0]?.sectionId));
    return <>
        <ErrorBoundary>
          <div className={classes.root}>
            <ClassmatesDialog userId={userId} selectedClasses={selectedClasses} userClasses={userClasses} close={this.closeClassmatesDialog} expertMode={expertMode} // state
          state={openClassmates} courseDisplayName={this.courseDisplayName()} />
            <HeaderNavigation selectedSectionIds={userClasses} setSelectedSectionIds={this.handleUpdateSelectedSectionIds} activeTab={activeTab} setActiveTab={this.handleSetActiveTab} firstName={firstName} schoolId={schoolId} classList={classList} openClassmatesDialog={this.openClassmatesDialog} pathname={pathname} expertMode={expertMode} push={push} />
            {selectedClasses.length === 1 && !selectedClasses[0].isCurrent ? null : <PostCreationHeader />}
            <FeedFilter query={query} from={from} userClasses={userClasses} expertMode={expertMode} courseDisplayName={courseName} postTypes={postTypes} classesList={processClasses({
            classes: classList
          })} classList={classList} newClassExperience={campaign.newClassExperience} fromDate={fromDate} toDate={toDate} onApplyFilters={this.handleApplyFilters} onOpenFilter={this.handleOpenFilter} onRefresh={this.refetchPosts} onChangeDateRange={this.handleChangeDateRange} onChangeSearch={this.handleChangeSearch} />
            <Tooltip id={9045} hidden={!expertMode} placement="right-start" text="When you're in Expert Mode, you see posts from all your classes at once.">
              <FeedList isLoading={isLoadingPosts} userId={userId} schoolId={schoolId} items={items} expertMode={expertMode} newClassExperience={campaign.newClassExperience} hasMore={hasMore} fromFeedId={fromFeedId} classList={classList} handleShare={this.handleShare} onPostClick={this.handlePostClick} onBookmark={this.handleBookmark} pushTo={push} onReport={this.handleReport} onDelete={this.handleDelete} onLoadMore={this.fetchMorePosts} onUserClick={this.handleUserClick} isFiltering={isFiltering} />
            </Tooltip>
          </div>
        </ErrorBoundary>
        <ErrorBoundary>
          <SharePost feedId={activeAction?.data} open={activeAction?.type === FEED_ACTIONS.SHARE} onClose={this.handleShareClose} />
        </ErrorBoundary>
        <ErrorBoundary>
          <Report open={activeAction?.type === FEED_ACTIONS.REPORT} ownerId={activeAction?.data.ownerId || ''} ownerName={activeAction?.data.ownerName || ''} objectId={activeAction?.data.feedId || -1} onClose={this.handleReportClose} />
        </ErrorBoundary>
        <ErrorBoundary>
          <DeletePost open={activeAction?.type === FEED_ACTIONS.DELETE} feedId={activeAction?.data} onClose={this.handleDeleteClose} />
        </ErrorBoundary>
      </>;
  }
