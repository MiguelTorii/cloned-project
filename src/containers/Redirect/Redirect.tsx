  ...
  render() {
    ...
    return <div className={classes.root}>
        <div className={classes.links}>
          ...
          {isIOS && <a className={classes.button} style={{
          textDecoration: 'none'
        }} rel="noopener noreferrer" target="_blank" href={`${osVersion.startsWith('13') ? IOS_13_REDIRECT_URI : IOS_REDIRECT_URI}?nonce=${nonce}`}>
              <Button variant="contained" color="primary">
                Open iOS App
              </Button>
            </a>}
          {isAndroid && <Link className={classes.button} href={`${ANDROID_REDIRECT_URI}?nonce=${nonce}`}>
              <Button variant="contained" color="primary" // onClick={this.handleAndroidClick(nonce)}
          >
                Open Android App
              </Button>
            </Link>}
