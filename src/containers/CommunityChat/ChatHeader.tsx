...
const ChatHeader = ({
  ...
  const isShow = useMemo(() => permission && permission.includes(PERMISSIONS.EDIT_GROUP_PHOTO_ACCESS) && permission.includes(PERMISSIONS.RENAME_GROUP_CHAT_ACCESS), [permission]);
  const temporaryAddMemberPermissionOfCommunityChat = useMemo(() => permission && permission.includes(PERMISSIONS.EDIT_GROUP_PHOTO_ACCESS), [permission]);
  const deletePermission = useMemo(() => permission && permission.includes(PERMISSIONS.REMOVE_USER_GROUP_CHAT_ACCESS), [permission]);
  const showThreeDotsMenu = useMemo(() => Object.keys(members).length > 2 && isShow || deletePermission, [isCommunityChat, deletePermission, members, isShow]);
  ...
  const renderOtherMembers = useCallback(() => <List component="nav" aria-label="secondary mailbox folders">
        {members.map((member, index) => {
      if (index > 2) {
        return <ListItem button key={member.userId}>
  ...
  return <div className={classes.header}>
      {channel && <Grid container justifyContent="space-between">
          <Typography className={classes.headerTitle}>
            ...
            {members.length > 3 && !channel.channelState.friendlyName && <ArrowDropDownIcon onClick={handleClick} />}
          </Typography>

          <Popover id={openMemberId} open={openMembers} anchorEl={membersEl} onClose={handleClose} anchorOrigin={{
        vertical: 'bottom',
        horizontal: 'right'
      }} transformOrigin={{
        vertical: 'top',
        horizontal: 'right'
          ...
          <div className={classes.chatIcons}>
            {(otherUser?.registered || memberKeys.length > 2) && videoEnabled && <IconButton aria-label="study-room" className={classes.chatIcon} onClick={startVideo}>
                <ChatStudyRoom />
              </IconButton>}
            {!isCommunityChat && <IconButton aria-label="add-member" className={classes.chatIcon} onClick={handleCreateChannelOpen}>
                <ChatAddMember />
              </IconButton>}
            {isCommunityChat && temporaryAddMemberPermissionOfCommunityChat && <IconButton aria-label="add-member" className={classes.chatIcon} onClick={handleCreateChannelOpen}>
                <ChatAddMember />
              </IconButton>}
            {Object.keys(members).length > 0 && <IconButton aria-label="studyroom-members" className={classes.chatIcon} onClick={onOpenRightPanel}>
                {rightSpace ? <ChatActiveStudyRoomMemberrs /> : <ChatStudyRoomMemberrs />}
              </IconButton>}
            {!isCommunityChat && <IconButton aria-label="share-link" className={classes.chatIcon} onClick={handleShareLink}>
                <IconShare className={classes.grayIcon} />
              </IconButton>}
            {showThreeDotsMenu && <IconButton aria-label="group-details" className={classes.chatIcon} onClick={handleOpenGroupDetailMenu}>
                <MoreVertIcon />
              </IconButton>}

            <Popover id={id} open={open} anchorEl={anchorEl} onClose={handleCloseGroupDetailMenu} anchorOrigin={{
          vertical: 'bottom',
          horizontal: 'right'
        }} transformOrigin={{
          vertical: 'top',
          horizontal: 'right'
              ...
              <Paper className={classes.paper}>
                <MenuList>
                  {Object.keys(members).length > 2 && isShow && <MenuItem onClick={handleEditGroup}>Edit Group Details</MenuItem>}
                  {deletePermission && <MenuItem onClick={handleRemoveStudent} className={classes.removeStudent}>
                      Remove Students
                    </MenuItem>}
                </MenuList>
              </Paper>
      ...
      <EditGroupDetailsDialog title="Edit Group Details" channel={channel} localChannel={local[channel.sid]} open={editGroupDetailsOpen} updateGroupName={handleUpdateGroupName} onClose={handleEditGroupDetailsClose} />
      <CreateChatChannelDialog title="ADD CLASSMATES" chatType={channelType} onClose={handleCreateChannelClose} onLoadOptions={handleLoadOptions} members={members} onSubmit={onSubmit} isLoading={loading} okLabel="Add Classmates" />
      <RemoveStudentDialog open={openRemoveStudent} onClose={handleCloseRemoveStudent} currentUserName={currentUserName} members={members} channel={channel} isCommunityChat />
      {channel && <ShareLinkDialog open={openShareLink} localChannel={local[channel.sid]} handleClose={closeShareLinkDialog} />}
    </div>;
};
