...
const CourseChannels = ({
  ...
  const handleGoToFeed = useCallback(() => {
    const communityClass = (userClasses.classList || []).find(item => (item.section || []).map(section => section.sectionId).includes(currentCommunity.section_id));
    ...
    dispatch(push(`/feed?class=${cypher(`${communityClass.classId}:${currentCommunity.section_id}`)}`));
  }, [currentCommunity, userClasses, dispatch]);
  return <Box>
      {currentCommunity.communityBannerUrl && <Box className={classes.courseLogo} display="flex" justifyContent="center" alignItems="center" mb={3}>
          <LoadImg url={currentCommunity.communityBannerUrl} className={classes.courseBanner} />
        </Box>}
      <Box className={cx(currentCommunity.communityBannerUrl ? classes.courseNameWithLogo : classes.courseName)} display="flex" justifyContent="center" alignItems="center" flexDirection="column" mb={3}>
      ...
      <CollapseNavbar channels={communityChannels} currentCommunityChannel={currentCommunityChannel} startMessageLoading={startMessageLoading} setCurrentCommunityChannel={setCurrentCommunityChannel} setCurrentChannelSidAction={setCurrentChannelSidAction} local={local} selectedChannel={selectedChannel} setSelctedChannel={setSelctedChannel} />
    </Box>;
};
