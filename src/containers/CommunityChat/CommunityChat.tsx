...
const CommunityChat = ({
  ...
  useEffect(() => {
    ...
    const filterCurrentCommunityChannel = allCommunityChannels.filter(communityChannel => communityChannel.courseId === currentCommunity.id);
    ...
    if (currentCommunity.id !== 'chat') {
      ...
      if (currentCommunityChannel) {
        ...
        const filterChannel = currentCommunityChannels.filter(channel => channel.chat_id === currentCommunityChannel.sid);
  ...
  const curSize = useMemo(() => width === 'xs' ? 12 : ['md', 'sm'].includes(width) ? 4 : 2, [width]);
  ...
  return <Grid className={classes.container} direction="row" container>
      ...
      {leftSpace !== 0 && <Grid item xs={leftSpace || 1} className={leftSpace !== 0 ? classes.left : classes.hidden}>
          {isLoading ? <Box className={classes.loading} display="flex" alignItems="center" justifyContent="center">
              <CircularProgress />
            </Box> : <CourseChannels setSelctedChannel={setSelctedChannel} selectedChannel={selectedChannel} startMessageLoading={startMessageLoading} communityChannels={communityChannels} local={local} currentCommunity={currentCommunity} currentCommunityChannel={currentCommunityChannel} setCurrentChannelSidAction={setCurrentChannelSidAction} setCurrentCommunityChannel={setCurrentCommunityChannel} />}
        </Grid>}
      {!isLoading && leftSpace !== 12 && <Grid item xs={12 - leftSpace - rightSpace} className={classes.main}>
          <Main isCommunityChat channelList={communityChannels} currentCommunity={currentCommunity} selectedChannel={selectedChannel} newMessage={newMessage} messageLoading={messageLoading} startMessageLoading={startMessageLoading} setMainMessage={setMainMessage} mainMessage={mainMessage} permission={permission} local={local} channel={currentCommunityChannel} selectedChannelId={selectedChannelId} onCollapseLeft={onCollapseLeft} onCollapseRight={onCollapseRight} setRightPanel={handleOpenRightPanel} user={user} rightSpace={rightSpace} showNotification={showNotification} />
        </Grid>}
      {!isLoading && <Grid item xs={rightSpace || 1} className={rightSpace !== 0 ? classes.right : classes.hidden}>
          <RightMenu isCommunityChat userId={userId} schoolId={schoolId} channel={currentCommunityChannel} local={local} />
        </Grid>}
    </Grid>;
};
