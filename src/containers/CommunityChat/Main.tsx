...
const Main = ({
  ...
  const hasPermission = useMemo(() => permission && permission.includes(PERMISSIONS.EDIT_GROUP_PHOTO_ACCESS), [permission]);
  ...
  const renderMessage = useCallback((item, profileURLs) => {
    ...
    try {
      switch (type) {
        ...
        case 'message':
        case 'own':
          return <ChatMessage key={id} role={role} isCommunityChat={isCommunityChat} date={item.date} isOnline={isOnline} isOwn={type === 'own'} currentUserId={userId} userId={item.author} members={channelMembers} isGroupChannl={members.length === 2} name={item.name} messageList={item.messageList} avatar={getAvatar({
            id: item.author,
            profileURLs
          })} onImageLoaded={handleScrollToBottom} onStartVideoCall={handleStartVideoCall} onImageClick={handleImageClick} handleBlock={handleBlock} />;
  ...
  }, [isCommunityChat, getIsOnline, getRole, handleImageClick, handleScrollToBottom, handleStartVideoCall, handleBlock, members, channelMembers, userId]);
  ...
  const loadingConversation = useCallback(() => <div className={classes.messageLoadingRoot}>
  ...
  const loadingErrorMessage = useCallback(() => <div className={classes.messageLoadingRoot}>
        <div className={classes.messageLoadingContainer}>
  ...
  return messageLoading || isLoading ? loadingConversation() : errorLoadingMessage ? loadingErrorMessage() : <div className={classes.root}>
      {channel && <ChatHeader isCommunityChat={isCommunityChat} channel={channel} currentUserName={`${firstName} ${lastName}`} title={isCommunityChat ? selectedChannel?.chat_name : local[channel.sid].title} rightSpace={rightSpace} otherUser={otherUser} memberKeys={memberKeys} startVideo={startVideo} videoEnabled={videoEnabled} local={local} onOpenRightPanel={setRightPanel} handleUpdateGroupName={handleUpdateGroupName} />}
      <div className={classes.messageRoot}>
        <div className={classes.messageContainer}>
          {!channelList.length && !isLoading && <EmptyMain otherUser={otherUser} noChannel={!channel} newChannel={newChannel} expertMode={expertMode} />}
          {channel && <InfiniteScroll className={classes.messageScroll} threshold={50} pageStart={0} loadMore={handleLoadMore} hasMore={hasMore} useWindow={false} initialLoad={false} isReverse>
              {!hasMore && <InitialAlert hasPermission={hasPermission} focusMessageBox={focusMessageBox} setFocusMessageBox={setFocusMessageBox} handleUpdateGroupName={handleUpdateGroupName} isCommunityChat={isCommunityChat} currentCommunity={currentCommunity} selectedChannel={selectedChannel} local={local} userId={userId} channel={channel} />}
        ...
        {channel && <MessageQuill isCommunityChat={isCommunityChat} value={value} userId={userId} setFiles={setFiles} files={files} focusMessageBox={focusMessageBox} getFullName={getFullName} onSendMessage={onSendMessage} onChange={handleRTEChange} showNotification={showNotification} setValue={setValue} handleClick={handleClick} onTyping={onTyping} showError={showError} />}
