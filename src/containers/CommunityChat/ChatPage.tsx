...
const ChatPage = ({
  ...
  const fetchCommunityChannels = async communities => {
    ...
    try {
      ...
      return channels.filter(channel => channel.length > 0).map(channel => ({
  ...
  useEffect(() => {
    async function fetchCommuniteis() {
      try {
        ...
        const nonEmptyCommunityIds = communityChannels.filter(channelGroup => channelGroup.channels.length > 0).map(channelGroup => channelGroup.courseId);
        const nonEmptyCommunities = communities.filter(community => nonEmptyCommunityIds.includes(community.community.id));
  ...
  useEffect(() => {
    if (oneTouchSendOpen || currentCommunityId === 'chat' || !currentCommunityId) {
      ...
      const targetCourse = communities.filter(course => course.community.id === currentCommunity.id);
      ...
      const targetCourseChannel = communities.filter(community => community.community.id === Number(currentCommunityId));

      if (targetCourseChannel.length) {
        setCurrentCommunity(targetCourseChannel[0].community);
      }
    }
  }, [currentCommunityId, setCurrentCommunityId, setCurrentCommunity, currentCommunity, communities, loading, campaign.chatLanding]);
  ...
  return <div className={classes.root}>
      <Box className={classes.collageList} direction="row">
        <CollageList local={local} unreadMessageCount={unreadMessageCount} selectedCourse={currentCommunity} communities={communities} communityChannels={communityChannels} handleSelect={handleSelect} />
      </Box>
