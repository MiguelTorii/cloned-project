...
const MessageQuill = ({
  ...
  const imageHandler = useCallback(async (imageDataUrl, type, imageData) => {
    ...
    try {
      const filename = ['image', '-', Math.floor(Math.random() * 1e12), '-', new Date().getTime(), '.', type.match(/^image\/(\w+)$/i)[1]].join('');
    ...
    if (quill) {
      ...
      quill.on('text-change', () => {
        if (quill.getSelection(true)) {
          ...
          const currentTooltipWidth = document.getElementById('message-toolbar') ? document.getElementById('message-toolbar').clientWidth : 0;
  ...
  useEffect(() => {
    async function sendMessage() {
      if (files.length === 0 && (value.trim() === '<p><br></p>' || !value || value.replaceAll('<p>', '').replaceAll('</p>', '').trim() === '')) {
        setValue('');
        setPressEnter(false);
      } else {
  ...
  return <div className={classes.messageQuill}>
      <div className={cx(files.length > 0 ? classes.editWithFile : classes.editor)} ref={inputFieldRef}>
        <div className={classes.innerContainerEditor}>
          <div className={classes.editorToolbar}>
            <div id="editor" className={classes.editorable} ref={quillRef} />
            <EditorToolbar id="message-toolbar" handleSelect={insertEmoji} handleUploadFile={handleUploadFile} />
          </div>
      ...
      {files.length > 0 && <div className={classes.files}>
          {files.map(file => <AttachFile key={file.url} file={file} onClose={() => onClose(file)} smallChat />)}
        </div>}
