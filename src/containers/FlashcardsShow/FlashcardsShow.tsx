...
import PostItemActions from "containers/PostItemActions/PostItemActions";
...
const FlashcardsShow = () => {
  ...
  // Memos
  const cardList = useMemo(() => (data.deck || []).map(item => ({
    id: item.id,
  ...
  const handleActionBookmark = useCallback(() => {
    dispatch(bookmarkFlashcards(me.userId, data.feedId, data.bookmarked, () => setData(update(data, {
      bookmarked: {
        $set: !data.bookmarked
      }
    }))));
  ...
  const handleClickUser = useCallback(() => {
    dispatch(push(buildPath(`/profile/${data.userId}`, {
  ...
  const DeckList = useMemo(() => <FlashcardsListEditor data={cardList} toolbarPrefix="show" readOnly />, [cardList]);
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} lg={8} xl={9}>
          ...
          <Box mt={2} display="flex" justifyContent="center" alignItems="center" position="relative">
            ...
            <Box ml={2}>
              <TransparentIconButton disabled={boardDeckIndex === data.deck.length - 1} onClick={handleNextDeck}>
                <IconNext />
              </TransparentIconButton>
            </Box>
  ...
  const renderAsFeed = () => <PostItem feedId={data.feedId} isFlashcard>
      <PostItemHeader hideShare feedId={data.feedId} classId={data.classId} expertMode={expertMode} pushTo={handlePush} router={router} pop={handleGoBack} postId={data.postId} typeId={data.typeId} currentUserId={me.userId} userId={data.userId} name={data.name} userProfileUrl={data.userProfileUrl} classroomName={data.courseDisplayName} created={data.created} body={data.summary} title={data.title} bookmarked={data.bookmarked} roleId={data.roleId} role={data.role} onBookmark={handleActionBookmark} onReport={handleReport} onDelete={handleDelete} onEdit={handleActionEdit} />
      <Box mt={3} />
      {renderBody()}
      <Box>
      ...
      <Box mt={3}>
        <PostItemActions userId={me.userId} ownerId={data.userId} feedId={data.feedId} postId={data.postId} typeId={data.typeId} name={data.name} userProfileUrl={data.profileImage} thanked={data.thanked} inStudyCircle={data.inStudyCircle} questionsCount={data.postInfo.questionsCount} thanksCount={data.postInfo.thanksCount} viewCount={data.postInfo.viewCount} ownName={`${me.firstName} ${me.lastName}`} onReload={reloadData} />
      </Box>
      <Box>
        <PostComments isPastClassFlashcard={pastClassIds.includes(data.classId)} feedId={data.feedId} postId={data.postId} typeId={data.typeId} classId={data.classId} readOnly={data.readOnly} />
      </Box>
    </PostItem>;
      ...
      <Grid container spacing={3} alignItems="center">
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item>
              <Link component="button" onClick={handleClickUser}>
                <Avatar initialText={getInitials(data.name)} src={data.userProfileUrl} desktopSize={45} mobileSize={45} />
              </Link>
            </Grid>
            <Grid item>
        ...
        <Grid item xs={12} lg={4} xl={3}>
          <Grid container spacing={2}>
            {me.userId === data.userId && !pastClassIds.includes(data.classId) && <Grid item>
                <IconActionButton onClick={handleActionEdit}>
                  <IconPen />
                </IconActionButton>
              </Grid>}
            <Grid item>
              <IconActionButton onClick={handleActionBookmark} disabled={isBookmarking} loading={isBookmarking}>
        ...
        <Grid item xs={12}>
          ...
          <Typography>
            {isShortSummary ? _.truncate(data.summary, {
            length: 50
          }) : data.summary}
            {data.summary.length > DESCRIPTION_LENGTH_THRESHOLD && <Link className={classes.moreLessLink} onClick={handleSummaryMoreOrLess} color="inherit" underline="always">
  ...
  return <Box>
      ...
      <Report open={isReportModalOpen} ownerId={data.userId} objectId={data.feedId} onClose={handleReportClose} />

      <DeletePost open={isDeleteModalOpen} feedId={data.feedId} onClose={handleDeleteClose} />

      <ShareLinkModal open={isShareModalOpen} link={`${APP_ROOT_PATH}/flashcards/${data.postId}`} title={<Typography variant="h6">
            <span role="img" aria-label="Two hands">
              🙌
            </span>
            &nbsp; You’re awesome for helping your peers! Ready to share a link to your{' '}
            <b>{data.title}</b> deck?
          </Typography>} onClose={handleCloseShareLinkModal} />

      <Dialog fullScreen open={isEditing} onCancel={handleCloseEditModal} TransitionComponent={SlideUp} id="flashcards-edit-modal">
        <FlashcardsDeckEditor flashcardId={data.postId} data={deckData} onAfterUpdate={handleEditFinish} />
      </Dialog>

      <Dialog fullScreen className={classes.reviewModal} contentClassName={classes.reviewModalContent} open={isReviewing} onCancel={handleCloseReviewModal} showHeader={false} TransitionComponent={SlideUp}>
        <FlashcardsReview flashcardId={data.postId} flashcardTitle={data.title} cards={cardList} onClose={handleCloseReviewModal} />
      </Dialog>

      <Dialog fullScreen className={classes.reviewModal} contentClassName={classes.reviewModalContent} open={isInQuiz} onCancel={handleCloseQuiz} showHeader={false} TransitionComponent={SlideUp}>
        <FlashcardsQuiz flashcardId={data.postId} cards={cardList} onClose={handleCloseQuiz} />
      </Dialog>

      <Dialog fullScreen className={classes.reviewModal} contentClassName={clsx(classes.reviewModalContent, classes.matchModalContent)} open={isInMatchGame} onCancel={handleCloseMatchGame} showHeader={false} TransitionComponent={SlideUp}>
        <FlashcardsMatchGame cards={cardList} flashcardId={flashcardId} flashcardTitle={data.title} onClose={handleCloseMatchGame} />
      </Dialog>
      <ScrollToTop />
    </Box>;
};
