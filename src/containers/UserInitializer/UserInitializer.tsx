...
import { LOGGED_IN_WIDGET_ID, LOGGED_OUT_WIDGET_ID, LOGGED_IN_WIDGET_URL, LOGGED_OUT_WIDGET_URL } from "./constants";
...
import { getChatLandingCampaign, getFlashcardsCampaign, getLeaderboardAndSupportCenterVisibilityCampaign } from "../../actions/campaign";
...
const UserInitializer = ({
  ...
  useEffect(() => {
    const oldScript = document.querySelector(`script[src="${userId ? LOGGED_OUT_WIDGET_URL : LOGGED_IN_WIDGET_URL}"]`);
  ...
  const renderForm = () => <>
      ...
      switch (field) {
        case 'first_name':
          return <TextValidator key={field} label="First Name" margin="normal" variant="outlined" onChange={handleChange('firstName')} name="firstName" autoComplete="firstName" autoFocus fullWidth value={currentUser.firstName} disabled={loading} validators={['required']} errorMessages={['first name is required']} />;

        case 'last_name':
          return <TextValidator key={field} label="Last Name" margin="normal" variant="outlined" onChange={handleChange('lastName')} name="lastName" autoComplete="lastName" autoFocus fullWidth value={currentUser.lastName} disabled={loading} validators={['required']} errorMessages={['last name is required']} />;

        case 'grade':
          return <SelectValidator key={field} value={currentUser.grade} fullWidth name="grade" label="Year" onChange={handleChange('grade')} variant="outlined" margin="normal" disabled={loading} validators={['required']} errorMessages={['Year is required']}>
                <MenuItem value="" />
                {(grades[segment] || []).map(item => <MenuItem key={item.value} value={item.value}>
                    {item.label}
                  </MenuItem>)}
              </SelectValidator>;

        case 'email':
          return <TextValidator key={field} label="Email" margin="normal" variant="outlined" onChange={handleChange('email')} name="email" autoComplete="email" autoFocus fullWidth value={currentUser.email} disabled={loading} validators={['required', 'isEmail']} errorMessages={['email is required', 'email is not valid']} />;
  ...
  return <ErrorBoundary>
      <Dialog open={open} disableEscapeKeyDown okTitle="Update" onOk={handleSubmit} showActions title="Update Profile">
      ...
      {windowWidth > 640 && <OnboardingPopup open={onboardingPopupOpen} updateOnboarding={updateOnboarding} userId={userId} />}
    </ErrorBoundary>;
};
...
export default connect(mapStateToProps, mapDispatchToProps)(withRoot(withStyles(styles)(UserInitializer)));