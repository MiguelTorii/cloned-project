...
const CommonBanner = ({
  ...
  useEffect(() => {
    const intervalID = setIntervalWithFirstCall(() => {
      ...
      if (duration.asMinutes() <= 0) {
        ...
        setDurationText([[duration.days(), 'day'], [duration.hours(), 'hour'], [duration.minutes(), 'minute']].map(item => pluralize(item[1], item[0], true)).join(', '));
      }
    }, 30 * INTERVAL.SECOND);
  ...
  return <div className={classes.body}>
      <div className={classes.image}>
        {isExpanded && announcement.imageUrl && <LoadImg url={announcement.imageUrl} style={{
        maxHeight: 64
      }} />}
      </div>
      ...
      <div className={classes.buttons}>
        ...
        label: classes.buttonLabel
          ...
          <Tooltip id={9088} placement="top-end" text="We’ve given out over $100,000 to students and every week we have a new give away. Check it out. " okButton="Yay! 🎉">
            <u> Learn More </u>
          </Tooltip>
        </Button>
        {isExpanded ? <Button onClick={() => setIsExpanded(false)}>
            <LoadImg url={minimizeSvg} style={{
          maxHeight: 10
        }} />
          </Button> : <Button onClick={() => setIsExpanded(true)}>
            <LoadImg url={expandSvg} style={{
          maxHeight: 10
        }} />
          </Button>}
      </div>
      <Dialog onCancel={() => setDialogOpen(false)} open={dialogOpen} contentClassName={classes.dialogContent} maxWidth="sm" className={classes.dialog} title={<div className={classes.dialogTitle}>
            {announcement.popupTitleImage && <LoadImg url={announcement.popupTitleImage} style={{
        marginRight: 10,
        maxHeight: 44,
        marginTop: 10
      }} />}
