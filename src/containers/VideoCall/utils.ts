...
export const applyAudioOutputDeviceSelection = (deviceId, audio) => typeof audio.setSinkId === 'function' ? audio.setSinkId(deviceId) : Promise.reject(new Error('This browser does not support setting an audio output device'));
      ...
      if (index > -1 && track) {
        return update(b, {
          [index]: {
            [track.kind]: {
              $apply: t => {
                const trackIndex = t.findIndex(item => local ? item.id === track.id : item.sid === track.sid);
