...
class TagsAutoComplete extends React.PureComponent<Props, State> {
  ...
  handleLoadOptions = async (search, loadedOptions, {
    page
  }) => {
  ...
  render() {
    ...
    return <ErrorBoundary>
        <div className={classes.root}>
          <AutoComplete values={tags} variant={variant || 'outlined'} isDisabled={isEdit} inputValue={inputValue} label="Tags (Optional)" placeholder="" error={error} errorText="You must add at least 1 tag" isMulti onChange={onChange} onLoadOptions={this.handleLoadOptions} />
        </div>
      </ErrorBoundary>;
  }
