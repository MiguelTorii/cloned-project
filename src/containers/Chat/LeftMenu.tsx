    ...
    root: classes.container
      ...
      root: classes.container
        ...
        root: classes.header
          ...
          <Grid container item justifyContent="space-between" alignItems="center" className={classes.headerTitle}>
            <Typography className={classes.title}>Chats</Typography>
            <Button variant="contained" classes={{
            root: classes.newButton
          }} color="primary" onClick={onNewChannel}>
              <img id="circlein-newchat" src={NewChannelIcon} alt="newChat" className={classes.imgIcon} />
            </Button>
          </Grid>
          <Grid item className={classes.gridItem}>
            <div className={classes.search}>
              <InputBase onChange={onChangeSearch} value={search || ''} placeholder="Search for a chat…" classes={{
        ...
        <Grid item className={classes.gridChatList}>
          ...
          <List className={classes.root}>
            ...
            {channelList.map(c => local[c] && <div key={local[c].sid} className={clsx(!searchChannels.includes(local[c].sid) && classes.hidden)}>
                    <ChatListItem selected={currentChannel && c === currentChannel.sid} channel={local[c]} userId={userId} onOpenChannel={onOpenChannel} handleRemoveChannel={handleRemoveChannel} handleMuteChannel={handleMuteChannel} dark />
                  </div>)}
          </List>
        </Grid>
      </Grid>
