...
const ClassmateQuestions = () => {
  ...
  return <>
      ...
      <Tabs value={selectedClass} onChange={handleChangeClass} textColor="inherit" variant="scrollable" scrollButtons="auto" classes={{
      root: classes.tabs
    }}>
        <Tab key="all" label="All Courses" value="all" />
        {myClasses.map(item => <Tab key={item.classId} label={item.className} value={item.classId} />)}
      </Tabs>
      <Box mb={3} />
