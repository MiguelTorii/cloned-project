...
const QuickNotes = ({
  ...
  const saveContent = useCallback(async (content, currentClass = null) => {
    ...
    if (!content || content === '<p><br></p>' || !selectedClass || !selectedClass.sectionId || !selectedClass.classId) {
      return;
    }
  ...
  const renderSaved = useMemo(() => {
    ...
    return <Tooltip id={3499} delay={600} hidden={!viewedOnboarding} placement="bottom" text="We save your QuickNotes with the date and time you created it.">
  ...
  const classList = useMemo(() => {
    if (userClasses?.classList) {
      const classList = userClasses.classList.filter(c => c.section.length !== 0).map(c => ({
        name: c.className,
        color: c.bgColor,
  ...
  return <Grid container>
      ...
      <Popover open={Boolean(anchorEl)} className={classes.container} anchorEl={anchorEl} onClose={handleClose} anchorOrigin={{
      vertical: 'bottom',
      horizontal: 'left'
    }} transformOrigin={{
      vertical: 'top',
      horizontal: 'right'
        ...
        <Paper className={classes.container} classes={{
        root: classes.quickNoteRoot
          ...
          <div className={classes.noteOptions}>
            <FormControl className={classes.formControl}>
              ...
              <Select className={classes.select} labelWidth={200} labelId="select-label" MenuProps={{
              anchorOrigin: {
                vertical: 'bottom',
                horizontal: 'left'
              },
            ...
            }} value={selectedClass?.sectionId || ''} renderValue={() => <Typography className={classes.renderMenu} style={{
              backgroundColor: selectedClass?.color
            }}>
                    {selectedClass.name}
                  </Typography>}>
                {classList.map(cl => <MenuItem key={cl.sectionId} onClick={() => updateClass(cl)} value={cl.sectionId} className={classes.menuItem} ListItemClasses={{
          ...
          <ReactQuill ref={quillRef} placeholder="Write all your brilliant ideas, a-ha moments, reminders and anything you need here 📝" theme="snow" value={quicknoteContent} onChange={handleUpdate} modules={{ ...modules,
          keyboard: {
            bindings
          }
        }} formats={formats} />
          <div className={classes.savedContainer}>
            {renderSaved}
            <Button variant="contained" disabled={!selectedClass || disableSaveNote} className={classes.button} classes={{
