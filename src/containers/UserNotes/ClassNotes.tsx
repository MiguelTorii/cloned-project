...
const ClassNotes = ({
  ...
  const renderTitleAndTooltip = useCallback((name, idx, onboardingOpen) => <div style={{
    display: 'flex'
  }}>
        {name}
        {idx === 0 && <Tooltip id={9002} hidden delay={600} placement="right" text="Select a class to view the notes inside, or to create new notes.">
            <div />
          </Tooltip>}
      </div>, []);
  ...
  const renderNotes = useCallback(() => {
    ...
    return <List className={classes.noteListRoot}>
        {noteList.length > 0 ? noteList.map(note => <Note noteData={note} key={note.id} />) : <div className={classes.emptyFolder}>
            No notes yet! Click below to add some amazing notes to study.
          </div>}
        <Button variant="text" className={classes.createNote} color="primary" onClick={handleCreateNote}>
  ...
  return <>
      <div className={classes.listItemContainer} onClick={handleSwitchExpand}>
        ...
        <ListItem className={classes.listItem}>
          ...
          <ListItemText primary={renderTitleAndTooltip(classData.className, arrayIndex, false)} className={classes.title} />
        </ListItem>
      </div>
      {renderNotes()}
    </>;
