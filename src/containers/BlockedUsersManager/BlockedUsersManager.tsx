...
class BlockedUsersManager extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    return <ErrorBoundary>
        <Dialog ariaDescribedBy="blocked-users-dialog-description" className={classes.dialog} onCancel={onClose} open={open} title="Blocked Users">
          {loading && <CircularProgress size={12} />}
          {!loading && blockedUsers.length === 0 && <Typography color="textPrimary" id="blocked-users-dialog-description">
              {"You don't have blocked users"}
            </Typography>}
          {!loading && <List className={classes.list}>
              {blockedUsers.map(user => <ListItem key={user.userId} dense>
                  <ListItemAvatar>
                  ...
                  <ListItemSecondaryAction>
                    <Button variant="contained" color="secondary" onClick={this.handleUnBlock(user.userId)}>
                      Unblock
                    </Button>
                  </ListItemSecondaryAction>
                </ListItem>)}
