...
const FloatingChat = ({
  ...
  const showNotification = useCallback(channel => {
  ...
  const handleMessageReceived = channel => () => <Button onClick={() => {
  ...
  useEffect(() => {
    if (local) {
      let unread = 0;
      const cl = Object.keys(local).filter(l => local[l].sid && !local[l]?.twilioChannel?.channelState?.attributes?.community_id).sort((a, b) => {
  ...
  useEffect(() => {
    const handleMessage = () => {
      ...
      if (Number(author) !== Number(userId) && !showNotification(channel) && !sids.includes(channel.sid) && !local[channel.sid].muted) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (updateOpenChannelsDebounce.cancel && typeof updateOpenChannelsDebounce.cancel === 'function') {
      ...
      if (client && profileImage !== '') {
        try {
          if (client.user.attributes.profileImageUrl !== profileImage) {
            client.user.updateAttributes({ ...client.user.attributes,
  ...
  return <>
      <ErrorBoundary>
        <div className={classes.root}>
          {openChannels.map(item => <ChatChannel key={`op${item.sid}`} push={push} setCurrentCommunityId={setCurrentCommunityId} getMembers={getMembers} user={user} channel={item} channels={channelList} setCurrentChannel={setCurrentChannel} localChannel={local[item.sid]} local={local} onClose={handleChannelClose} onRemove={handleRemoveChannel} onBlock={handleBlockUser} onSend={() => {
          if (onboardingListVisible) {
            setTimeout(() => getOnboardingList(), 1000);
          }
        }} />)}
          <ErrorBoundary>
            {newChannel && <ChatChannel push={push} user={user} setCurrentCommunityId={setCurrentCommunityId} onClose={handleNewChannelClose} newChannel handleChannelCreated={handleChannelCreated} channel={{
            sid: '',
          ...
          <MainChat unread={unread} onCreateChannel={handleNewChannelOpen}>
            {channelList.length === 0 ? <div className={classes.noMessages}>
                {!isLoading ? <>
              ...
              </div> : channelList.map(c => local[c] && <ChatListItem key={local[c].sid} channel={local[c]} userId={userId} handleMuteChannel={handleMuteChannel} handleRemoveChannel={handleRemoveChannel} onOpenChannel={onChannelOpen} />)}
          </MainChat>
        </div>
      </ErrorBoundary>
      <ErrorBoundary>
        <CreateChatChannel type={createChannel} client={client} channels={channels} onClose={handleCreateChannelClose} onChannelCreated={handleChannelCreated} />
      </ErrorBoundary>
    </>;
};
...
export default connect(mapStateToProps, mapDispatchToProps)(withRoot(withStyles(styles)(FloatingChat)));