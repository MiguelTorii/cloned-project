...
const WeeklyGoals = () => {
  ...
  return <Paper elevation={0} square={false} className={classes.root}>
      ...
      {loading ? <LoadingSpin /> : <>
          <Grid container spacing={3}>
            {goals.map(item => <Grid item key={item.id} className={classes.goalItem}>
                <StudyGoalProgress title={item.display_name} content={<img className={classes.studyGoalImage} src={item.icon_url} alt={item.display_name} title={item.display_name} />} value={item.current} total={item.goal} />
              </Grid>)}
          </Grid>
          <Box display="flex" justifyContent="center" mt={3} mb={1}>
            {modalStatus.id > 0 && <Link component="button" underline="none" onClick={handleReportOpenModal} className={classes.reportButton}>
                {modalStatus.id > 3 ? 'View End of Week Report' : 'View Mid-Week Report'}
              </Link>}
          </Box>
          <HelpModal open={isHelpModalOpen} onClose={handleCloseHelpModal} />
          {modalStatus.id > 0 && <ReportModal data={ModalData[modalStatus.id]} open={isReportModalOpen} onClose={handleCloseReportModal} value={modalStatus.current} total={modalStatus.goal} />}
        </>}
    </Paper>;
};
