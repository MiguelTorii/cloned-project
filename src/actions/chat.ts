...
import { getGroupMembers, getShareLink, getChannels, muteChannel, unmuteChannel, renewTwilioToken, leaveChat, blockChatUser, createChannel } from "api/chat";
...
const setCurrentCommunityAction = ({
  channel
}: {
  channel: Record<string, any>;
}) => ({
...
const setCurrentCommunityIdAction = ({
  currentCommunityId
}) => ({
...
export const setCurrentCommunity = channel => async (dispatch: Dispatch) => {
  if (channel) {
    dispatch(setCurrentCommunityAction({
      channel
    }));
  }
};
...
export const setCurrentChannel = currentChannel => async (dispatch: Dispatch) => {
  if (currentChannel) {
    ...
    const [members, shareLink] = await Promise.all([fetchMembers(currentChannel.sid), getShareLink(currentChannel.sid)]);
...
export const setCurrentCommunityChannel = currentChannel => async (dispatch: Dispatch) => {
  if (currentChannel) {
    ...
    dispatch(setCurrentCommunityChannelAction({
      currentChannel
    }));
    dispatch(updateShareLink({
      shareLink,
...
export const setCommunityChannels = communityChannels => (dispatch: Dispatch) => {
  dispatch(setCommunityChannelsAction({
    communityChannels
  }));
};
...
export const handleNewChannel = newChannel => (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  const {
    chat: {
      data: {
        openChannels
...
const initLocalChannels = async (dispatch, currentLocal = {}) => {
  try {
    ...
    if (Object.keys(local).length > 0 && Object.keys(currentLocal).length > 0 && !localStorage.getItem('currentDMChannel')) {
      let channelList = [];
      ...
      if (recentMessageChannels.length) {
        channelList = recentMessageChannels.sort((a, b) => moment(local[b].lastMessage.date).valueOf() - moment(local[a].lastMessage.date).valueOf());
      }
...
export const openChannelWithEntity = ({
  ...
  if (!fullscreen) {
    dispatch(requestStartChannelWithEntity({
      entityId,
      entityFirstName,
      entityLastName,
      entityVideo,
    ...
    if (channel) {
      ...
      const [members, shareLink] = await Promise.all([fetchMembers(channel.sid), getShareLink(channel.sid)]);
      dispatch(updateMembers({
        members,
...
export const handleInitChat = () => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  const {
    user: {
      data: {
        userId
  ...
  try {
    ...
    if (client._eventsCount === 0) {
      ...
      client.on('channelUpdated', async ({
        channel
      }) => {
...
export const handleShutdownChat = () => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  const {
    chat: {
      data: {
        client,
...
export const handleUpdateGroupPhoto = (channelSid: string, image: Blob, callback: (...args: Array<any>) => any) => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  const {
    user: {
      data: {
        userId
...
export const handleRoomClick = channel => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  const {
    chat: {
      data: {
        openChannels
  ...
  try {
    ...
    dispatch(setOpenChannels({
      openChannels: newState
    }));
  } catch (err) {}
};
export const updateOpenChannels = () => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  const {
    chat: {
      data: {
        openChannels
...
export const handleChannelClose = (sid: string) => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  const {
    chat: {
      data: {
        openChannels
...
export const handleUpdateFriendlyName = (channel: Record<string, any>) => async (dispatch: Dispatch) => {
  dispatch(updateFriendlyName({
    channel
  }));
};