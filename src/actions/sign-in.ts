...
export const updateUser = ({
  ...
  const isExpert = user.permission.includes(PERMISSIONS.EXPERT_MODE_ACCESS) && user.permission.includes(PERMISSIONS.MAIN_APPLICATION_ACCESS);
  const isTutor = user.permission.includes(PERMISSIONS.EXPERT_MODE_ACCESS) && user.permission.indexOf(PERMISSIONS.MAIN_APPLICATION_ACCESS) === -1;
...
export const checkUserSession = () => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  try {
  ...
  if (!['/new', '/reset_password', '/old', '/oauth', '/auth', '/forgot_password', '/reset_password', '/terms-of-use', '/redirect', '/saml', '/gondor'].includes(pathname) && !pathname.includes('/canvas') && !deepLinkCheck(pathname)) {
    dispatch(push('/'));
  }
