...
export const updateQuickNoteContent = ({
  ...
  dispatch(updateQuickNoteContentAction({
    content
  }));
};
...
export const getNotes = () => async (dispatch: Dispatch, getState: (...args: Array<any>) => any) => {
  try {
    const {
      notes: {
        data: {
